 ...
 * @typedef {Object}
ConfigLoaderOptions
 * @property {string|false|undefined}
configFile The path to the config file to use.
 * @property {string}
cwd The current working directory.
 * @property {boolean}
ignoreEnabled Indicates if ignore patterns should be honored.
 * @property {Config|Array<Config>}
[baseConfig] The base config to use.
 * @property {Array<Config>}
[defaultConfigs] The default configs to use.
 * @property {Array<string>}
[ignorePatterns] The ignore patterns to use.
 * @property {Config|Array<Config>}
[overrideConfig] The override config to use.
 * @property {boolean}
[hasUnstableNativeNodeJsTSConfigFlag] The flag to indicate whether the `unstable_native_nodejs_ts_config` flag is enabled.
...
loaderOptions The options to use when loading configuration files.
 * @returns {void}
...
filePath The file path to check.
 * @returns {boolean}
`true` if the file is a TypeScript file, `false` if it's not.
 */
 ...
 * @returns {boolean}
`true` if the ESLint is running Bun, `false` if it's not.
 */
 ...
 * @returns {boolean}
`true` if the ESLint is running in Deno, `false` if it's not.
 */
 ...
 * {@linkcode process.features}
object
 * and if its value is either "strip" or "transform".
 * @returns {boolean}
`true` if native TypeScript support is enabled, otherwise `false`.
 * @since 9.24.0
 */
 ...
 * @param {string}
filePath The absolute file path to load.
 * @param {URL}
 ...
 * @param {string}
filePath The absolute file path of the module to import.
 * @param {URL}
fileURL The file URL to load.
 * @param {number}
mtime The last modified timestamp of the file.
 * @returns {Promise<any>}
- A {@linkcode Promise | promise} that resolves to the imported ESLint config.
 * @since 9.24.0
 */
 ...
 * @param {string}
filePath The filename to load from.
 * @param {boolean}
hasUnstableNativeNodeJsTSConfigFlag The flag to indicate whether the `unstable_native_nodejs_ts_config` flag is enabled.
 * @returns {Promise<any>}
The config loaded from the config file.
 */
...
|Promise<{configFilePath:string,basePath:string}>>}
	 */
...
*/
	 ...
	 * @param {ConfigLoaderOptions}
options The options to use when loading configuration files.
	 */
	constructor(options) {
	 ...
	 * @param {string}
fromDirectory The directory from which to start searching.
...
/**
	 ...
	 * @param {string}
configFilePath The absolute path to the config file to use if not overridden.
	 * @param {string}
basePath The base path to use for relative paths in the config file.
	 * @returns {Promise<FlatConfigArray>}
The config array for `eslint`.
	 */
...
/**
	 ...
	 * @param {string}
fileOrDirPath The file or directory path to get the config file path for.
	 * @returns {Promise<string|undefined>}
The config file path or `undefined` if not found.
	 * @throws {Error}
If `fileOrDirPath` is not a non-empty string.
	 * @throws {Error}
If `fileOrDirPath` is not an absolute path.
	 */
...
/**
	 ...
	 * @param {string}
filePath The path of the file or directory to retrieve config for.
	 * @returns {Promise<FlatConfigArray>}
A configuration object for the file.
	 * @throws {Error}
If no configuration for `filePath` exists.
	 */
...
/**
	 ...
	 * configuration for each file it processes.
	 * @param {string}
dirPath The path of the directory to retrieve config for.
	 * @returns {Promise<FlatConfigArray>}
A configuration object for the directory.
	 */
...
=
		...
		debug(`Using config file ${configFilePath}
...
If `filePath` is not an absolute path.
	 * @throws {Error}
If the config file was not already loaded.
	 */
...
/**
	 ...
	 * been loaded and we just need to get the configuration for a file.
	 * @param {string}
fileOrDirPath The path of the directory to retrieve a config object for.
	 * @returns {FlatConfigArray}
A configuration object for the directory.
	 * @throws {Error}
If `dirPath` is not a non-empty string.
	 * @throws {Error}
...
const configFilePathInfo = this.#configFilePaths.get(absoluteDirPath)
		...
		if (typeof configFilePathInfo.then === "function") {
			throw new Error(
				`Config file path for ${fileOrDirPath}
has not yet been calculated or an error occurred during the calculation`,
			)
		}
const { configFilePath } = configFilePathInfo
		...
		if (!configArray || typeof configArray.then === "function") {
			throw new Error(
				`Config array for ${fileOrDirPath}
has not yet been calculated or an error occurred during the calculation`,
			)
		}
return configArray
...
/**
	 ...
	 * @returns {Promise<{createJiti: Function|undefined, version: string;}
>} A promise that fulfills with an object containing the jiti module's createJiti function and version.
	 */
...
[options] the options object
	 * @param {string|false|undefined}
options.useConfigFile The path to the config file to use.
	 * @param {string}
options.cwd Path to a directory that should be considered as the current working directory.
	 * @param {string}
[options.fromDirectory] The directory from which to start searching. Defaults to `cwd`.
	 * @returns {Promise<{configFilePath:string|undefined,basePath:string}
>} Location information for
	 *      the config file.
	 */
...
= options
		...
		debug(
			`Calculating config array from config file ${configFilePath}
and base path ${basePath}`,
		)
...
is ${emptyConfig ? "empty" : "not empty"}`,
			)
...
*/
	 ...
	 * @type {Promise<{configFilePath:string,basePath:string}
|undefined>}
	 */
	#configFilePath
...
/**
	 ...
	 * @param {string}
dirPath The directory path to get the config file path for.
	 * @returns {Promise<string|undefined>}
The config file path or `undefined` if not found.
	 * @throws {Error}
If `fileOrDirPath` is not a non-empty string.
...
=
		...
		debug(
			`[Legacy]: Using config file ${configFilePath}
and base path ${basePath}`,
		)
		return this.#calculateConfigArray(configFilePath, basePath)
	}
/**
	 ...
	 * been loaded and we just need to get the configuration for a file.
	 * @param {string}
dirPath The path of the directory to retrieve a config object for.
	 * @returns {FlatConfigArray}
A configuration object for the file.
	 * @throws {Error}
If `dirPath` is not a non-empty string.
