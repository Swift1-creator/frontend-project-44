...
const { flatConfigSchema, hasMethod }
 ...
 * @param {string}
ruleId.pluginName The ID of the rule to find.
 * @param {string}
ruleId.ruleName The ID of the rule to find.
 * @param {Object}
config The config to search in.
 * @throws {TypeError}
For missing plugin or rule.
 * @returns {void}
*/
function throwRuleNotFoundError({ pluginName, ruleName }
, config) {
	const ruleId = pluginName === "@" ? ruleName : `${pluginName}
/${ruleName}`

	const errorMessageHeader = `Key "rules": Key "${ruleId}
"`

	let errorMessage = `${errorMessageHeader}
: Could not find plugin "${pluginName}" in configuration.`
...
: Rule "${ruleName}" was removed and replaced by "${replacementRuleName}".`
		}
else {
			errorMessage = `${errorMessageHeader}
: Could not find "${ruleName}" in plugin "${pluginName}".`
			...
			for (const [otherPluginName, otherPlugin] of Object.entries(
				...
				if (otherPlugin.rules && otherPlugin.rules[ruleName]) {
					errorMessage += ` Did you mean "${otherPluginName}
/${ruleName}"?`
					break
				}
}
...
class InvalidRuleOptionsSchemaError extends Error {
	 ...
	 * @param {string}
ruleId Id of the rule that has an invalid `meta.schema`.
	 * @param {Error}
processingError Error caught while processing the `meta.schema`.
	 */
	constructor(ruleId, processingError) {
		super(
			`Error while processing options validation schema of rule '${ruleId}
 ...
 * @param {string}
ruleId The rule ID to parse.
 * @returns {{pluginName:string,ruleName:string}
} The plugin and rule
 *      parts of the ruleId
 */
 ...
 * @param {string}
ruleId The rule ID to look for.
 * @param {Linter.Config}
config The config to search.
...
rule A rule object
 * @throws {TypeError}
If `meta.schema` is specified but is not an array, object or `false`.
 * @returns {Object|null}
JSON Schema for the rule's options. `null` if `meta.schema` is `false`.
 */
function getRuleOptionsSchema(rule) {
	if (!rule.meta) {
		return { ...noOptionsSchema }
; // default if `meta.schema` is not specified
	}
 ...
 * @returns {{objectName: string, pluginName: string}
 ...
 * @returns {string?}
The name of the object if found or `null` if there
 *      is no name.
 */
 ...
 * @param {string}
objectKey The key of the object being checked.
 * @returns {void}
...
function assertNotFunction(value, key, objectKey) {
	if (typeof value === "function") {
		const error = new TypeError(
			`Cannot serialize key "${key}
" in "${objectKey}": Function values are not supported.`,
		)
...
languageOptions The options to create a JSON
 *     representation of.
 * @param {string}
objectKey The key of the object being converted.
 * @returns {Record<string, any>}
The JSON representation of the languageOptions.
 * @throws {TypeError}
If a function is found in the languageOptions.
 */
 ...
 * @param {Object}
rule The rule to get a validator for.
 * @param {string}
ruleId The ID of the rule (for error reporting).
 * @returns {Function|null}
A validation function or null if no validation is needed.
 * @throws {InvalidRuleOptionsSchemaError}
If a rule's `meta.schema` is invalid.
 */
...
= splitPluginIdentifier(language)
		...
		if (
			...
			throw new TypeError(
				`Key "language": Could not find "${localLanguageName}
" in plugin "${languagePluginName}".`,
			)
		}
this.language =
			plugins[languagePluginName].languages[localLanguageName]
...
=
				...
				if (
					...
					throw new TypeError(
						`Key "processor": Could not find "${localProcessorName}
...
return `${namespace}:${pluginId}`
	 ...
	 * @param {string}
ruleId The ID of the rule to get.
	 * @returns {RuleDefinition|undefined}
The rule definition from the plugin, or `undefined` if the rule is not found.
	 */
...
this.rules[ruleId] = ruleConfig
	 ...
	 * @param {Object}
rulesConfig The rules config to validate.
	 * @returns {void}
* @throws {Error} If a rule's configuration does not match its schema.
	 * @throws {TypeError}
If the rulesConfig is not provided or is invalid.
	 * @throws {InvalidRuleOptionsSchemaError}
If a rule's `meta.schema` is invalid.
	 * @throws {TypeError}
If a rule is not found in the plugins.
	 */
...
"`)
									...
									return `\tValue ${JSON.stringify(error.data)}
...
`,
	 ...
	 * Gets a complete options schema for a rule.
	 * @param {RuleDefinition}
ruleDefinition A rule definition object.
	 * @throws {TypeError}
If `meta.schema` is specified but is not an array, object or `false`.
	 * @returns {Object|null}
JSON Schema for the rule's options. `null` if `meta.schema` is `false`.
...
ruleConfig A rule's configuration value, generally
	 * received from the user. A valid config value is either 0, 1, 2, the string "off" (treated the same as 0),
	 * the string "warn" (treated the same as 1), the string "error" (treated the same as 2), or an array
	 * whose first element is one of the above values. Strings are matched case-insensitively.
	 * @returns {(0|1|2)}
The numeric severity value if the config value was valid, otherwise 0.
	 */
