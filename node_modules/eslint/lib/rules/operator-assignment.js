 ...
 * @returns {boolean}
True if the operator is commutative and has a
 *     shorthand form.
 */
...
operator Operator to check.
 * @returns {boolean}
True if the operator is not commutative and has
 *     a shorthand form.
 */
 ...
 * @param {ASTNode}
node The node on the left side of the expression
 * @returns {boolean}
`true` if the node can be fixed
 */
...
,
		...
		messages: {
			...
			unexpected:
				"Unexpected operator assignment ({{operator}
}) shorthand.",
		}
,
	}
,
	...
	create(context) {
		 ...
		 * @param {ASTNode}
node An AssignmentExpression or BinaryExpression node
		 * @returns {Token}
The operator token in the node
		 */
...
// If this change would modify precedence (e.g. `foo *= bar + 1` => `foo = foo * (bar + 1)`), parenthesize the right side.
							if (
								...
								rightText = `${sourceCode.text.slice(operatorToken.range[1], node.right.range[0])}
...
return fixer.replaceText(
								node,
								`${leftText}
= ${leftText}${newOperator}${rightText}`,
							)
						}
return null
					}
