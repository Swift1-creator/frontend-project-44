...
node node to check
 * @returns {boolean}
True if the node is callee of a `NewExpression` node
 * @private
 */
...
node node to evaluate
		 * @returns {boolean}
True if it is wrapped in any parens
		 * @private
		 */
...
node node to evaluate
		 * @returns {boolean}
True if it is wrapped in grouping parens
		 * @private
		 */
...
node node to evaluate
		 * @returns {ASTNode}
node that is the function expression of the given IIFE, or null if none exist
		 */
					...
					context.report({
						...
						fix(fixer) {
							...
							if (
								 ...
								 * for example, in `(function(foo) {}
(bar))`, the range `(bar))` should get replaced with `)(bar)`.
								 */
...
,
					...
					context.report({
						...
						fix(fixer) {
							 ...
							 * and the call expression. For example, in `(function(foo) {}
)(bar)`, the range `)(bar)`
