...
node node to check
 * @returns {boolean}
`true` if the given node is in a statement list
 */
...
,
		...
		messages: {
			combineUninitialized:
				"Combine this with the previous '{{type}
}' statement with uninitialized variables.",
			combineInitialized:
				"Combine this with the previous '{{type}
}' statement with initialized variables.",
			splitUninitialized:
				"Split uninitialized '{{type}
}' declarations into multiple statements.",
			splitInitialized:
				"Split initialized '{{type}
}' declarations into multiple statements.",
			splitRequires:
				"Split requires to be separated into a single block.",
			combine: "Combine this with the previous '{{type}
}' statement.",
			split: "Split '{{type}
}' declarations into multiple statements.",
		}
...
/**
		 ...
		 * @param {ASTNode}
decl variable declaration Node
		 * @returns {bool}
if decl is a require, return true; else return false.
		 * @private
		 */
...
/**
		 ...
		 * @param {string}
statementType one of: "var", "let", "const", "using", or "awaitUsing"
		 * @param {ASTNode[]}
declarations List of declarations
		 * @param {Object}
currentScope The scope being investigated
		 * @returns {void}
* @private
		 */
...
statementType one of: "var", "let", "const", "using", or "awaitUsing"
		 * @returns {Object}
The scope associated with statementType
		 */
...
declarations List of declarations
		 * @returns {Object}
Counts of 'uninitialized' and 'initialized' declarations
		 * @private
		 */
...
declarations List of declarations
		 * @returns {boolean}
Returns true if it is the first var declaration, false if not.
		 * @private
		 */
...
declarations The `VariableDeclaration` to join
		 * @returns {Function}
The fixer function
		 */
...
declaration The `VariableDeclaration` to split
...
/**
		 ...
		 * @param {ASTNode}
node The VariableDeclaration node to check
		 * @returns {void}
* @private
		 */
