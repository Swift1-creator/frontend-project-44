 ...
 * @returns {boolean}
`true` if the node is 'NaN' identifier.
 */
...
,
		...
		messages: {
			...
			indexOfNaN:
				"Array prototype method '{{ methodName }
}' cannot find NaN.",
...
,
	...
	create(context) {
		const [{ enforceForIndexOf, enforceForSwitchCase }
] = context.options
...
wrapValue A function that wraps the compared value with a fix.
		 * @returns {function(Fixer): Fix}
The fixer function.
		 */
...
const fixedValue = wrapValue(comparedValueText)
				...
				return fixer.replaceText(node, `${negation}
${fixedValue}`)
			}
}
