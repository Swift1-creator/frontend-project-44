 ...
 * @param {import('eslint-scope').Scope}
scope Scope of the expression
 * @returns {boolean}
True iff "undefined" or "void ..."
 */
...
expression Expression to check
 * @returns {boolean}
True for identifiers and member expressions
 */
...
scope Scope of the expression
 * @returns {boolean}
True iff implicit nullish comparison
 */
...
scope Scope of the expression
 * @returns {boolean}
True iff explicit nullish comparison
 */
...
scope Scope of the expression
 * @returns {boolean}
Whether the expression is a boolean cast
 */
 ...
 * @param {import('eslint-scope').Scope}
scope Scope of the expression
 * @returns {?{ reference: ASTNode, operator: '??'|'||'|'&&'}
} Null if not a known existence
 */
...
node Node to check
 * @returns {boolean}
True iff passed node is inside a with block
 */
...
,
		...
		messages: {
			...
			unexpected:
				"Unexpected logical operator assignment ({{operator}
}) shorthand.",
			separate:
...
node reference
		 * @returns {boolean}
True iff a single property is accessed
		 */
...
/**
		 ...
		 * @param {{ messageId: string, node: ASTNode }
} descriptor Report descriptor without fix or suggest
		 * @param {{ messageId: string, fix: Function }
} suggestion Adds the fix or the whole suggestion as only element in "suggest" to suggestion
		 * @param {boolean}
shouldBeFixed Fix iff the condition is true
		 * @returns {Object}
Descriptor with either an added fix or suggestion
		 */
		 ...
		 * @param {ASTNode}
node AssignmentExpression or BinaryExpression
		 * @returns {import('eslint').AST.Token}
Operator token between the left and right expression
		 */
