 ...
 * @returns {boolean}
`true` if the node is `ForStatement` update.
 */
 ...
 * @returns {boolean}
`true` if the node is a for loop afterthought.
 */
...
,
		...
		messages: {
			unexpectedUnaryOp: "Unary operator '{{operator}
}' used.",
		}
,
	}
,
	...
	create(context) {
		const [{ allowForLoopAfterthoughts }
] = context.options
