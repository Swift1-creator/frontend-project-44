...
Key is the name of the variable. Value is `true` if the variable is considered writable, `false` otherwise.
 */
 ...
 * @param {Reference}
reference A reference to check.
 * @param {number}
index The index of the reference in the references.
 * @param {Reference[]}
references The array that the reference belongs to.
 * @returns {boolean}
Success/Failure
 * @private
 */
 ...
 * @returns {boolean}
`true` if the string starts with uppercase.
 */
 ...
 * @param {ASTNode}
node A function node to check.
 * @returns {boolean}
Whether or not a node is a constructor.
 */
 ...
 * @returns {boolean}
`true` if the node is a function node.
 */
...
node A node to check.
 * @returns {boolean}
`true` if the node is a loop node.
 */
...
node The node to check
 * @returns {boolean}
`true` if the node is a `null` literal
 */
...
node A node to check.
 * @returns {boolean}
Whether or not the node is a `null` or `undefined`.
 * @public
 */
...
node A node to check.
 * @returns {boolean}
Whether or not the node is callee.
 */
 ...
 * @returns {string|null}
The property name if static. Otherwise, null.
 */
 ...
 * @returns {ASTNode}
The `ChainExpression#expression` value if the node is a `ChainExpression` node. Otherwise, the node.
 */
 ...
 * @param {string}
actual The string value to check.
 * @param {string | RegExp}
expected The expected string value or pattern.
 * @returns {boolean}
`true` if the `actual` is an expected value.
 */
...
node The node to check.
 * @param {string | RegExp}
name The expected name or the expected pattern of the object name.
 * @returns {boolean}
`true` if the node is an Identifier node with the name.
 */
...
objectName The expected name or the expected pattern of the object name. If this is nullish, this method doesn't check object.
 * @param {string | RegExp | null}
propertyName The expected name or the expected pattern of the property name. If this is nullish, this method doesn't check property.
 * @returns {boolean}
`true` if the node is member access with the object name and property name pair.
 ...
 * @param {ASTNode}
left The Literal node to compare.
 * @param {ASTNode}
right The other Literal node to compare.
 * @returns {boolean}
`true` if the two literal nodes are the same value.
 */
 ...
 * @param {boolean}
[disableStaticComputedKey] Don't address `a.b` and `a["b"]` are the same if `true`. For backward compatibility.
 * @returns {boolean}
`true` if both sides match and reference the same value.
 */
...
node A node to check.
 * @returns {boolean}
Whether or not the node is a `Reflect.apply`.
 */
...
node A node to check.
 * @returns {boolean}
Whether or not the node is a `Array.from`.
 */
...
node A node to check.
 * @returns {boolean}
Whether or not the node is a method which expects a function as a first argument, and `thisArg` as a second argument.
 */
...
node A node to check.
 * @param {SourceCode}
sourceCode A SourceCode instance to get comments.
 * @returns {boolean}
Whether or not the node has a `@this` tag in its comments.
 */
 ...
 * @param {SourceCode}
sourceCode The ESLint source code object
 * @param {ASTNode}
node The node to be checked.
 * @returns {boolean}
True if the node is parenthesised.
 * @private
 */
...
token The token to check.
 * @returns {boolean}
`true` if the token is a dot token.
 */
...
token The token to check.
 * @returns {boolean}
`true` if the token is a keyword token.
 */
 ...
 * @param {ASTNode}
left A node 1 to compare.
 * @param {ASTNode}
right A node 2 to compare.
 * @param {SourceCode}
sourceCode The ESLint source code object.
 * @returns {boolean}
 ...
 * @param {ASTNode}
left A node to check.
 * @param {ASTNode}
right Another node to check.
 * @returns {boolean}
`true` if the two nodes are the pair of a logical expression and a coalesce expression.
 */
 ...
 * @returns {boolean}
`true` if the operator is a logical assignment operator.
 */
 ...
 * @param {ASTNode}
node The SwitchCase node to get.
 * @param {SourceCode}
sourceCode The source code object to get tokens.
 * @returns {Token}
The colon token of the node.
 */
 ...
 * @param {ASTNode}
node `Identifier` or string `Literal` node in a position
 ...
 * @param {ASTNode}
node any `Literal` node
 * @returns {boolean | null}
`true` when node is truthy, `false` when node is falsy,
 ...
 * @param {ASTNode}
node The branch of main condition which needs to be checked
 * @param {string}
operator The operator of the main LogicalExpression.
 * @returns {boolean}
true when condition short circuits whole condition
 */
 ...
 * @param {Scope}
scope The scope in which the identifier is referenced.
 * @param {ASTNode}
node An identifier node to check.
 * @returns {boolean}
`true` if the identifier is a reference to a global variable.
 */
...
node The node to check.
 * @returns {boolean}
Whether or not the node is an ExpressionStatement at the top level of a
...
node The node to check.
 * @returns {boolean}
`true` if the node is a part of directive prologue.
 */
 ...
 * @param {ASTNode}
node A node at the position where an opening parenthesis or bracket will be inserted.
 * @returns {boolean}
Whether a semicolon is required before the opening parenthesis or bracket.
 */
...
node The node to check.
	 * @returns {boolean}
Whether the node is inside a TypeScript type context.
	 */
...
node The node to check.
 * @returns {boolean}
Whether the node is used as an import attribute key.
 */
...
module.exports = {
	 ...
	 * @param {Object}
left The left token object.
	 * @param {Object}
right The right token object.
	 * @returns {boolean}
Whether or not the tokens are on the same line.
	 * @public
	 */
...
node A node to check.
	 * @returns {boolean}
`true` if the node is a string literal.
	 */
...
references An array of references.
	 * @returns {Reference[]}
An array of only references which are non initializer and writable.
	 * @public
	 */
...
,
	 ...
	 * @param {string}
val The text to check.
	 * @param {string}
character The character to see if it's surrounded by.
	 * @returns {boolean}
True if the text is surrounded by the character, false if not.
	 * @private
	 */
...
,
	 ...
	 * @param {Line|Block}
node The comment token to be checked
	 * @returns {boolean}
`true` if the node is an ESLint directive comment
	 */
...
The trailing statement's node.
	 */
	getTrailingStatement: esutils.ast.trailingStatement,

	/**
	 * Finds the variable by a given name in a given scope and its upper scopes.
	 * @param {eslint-scope.Scope}
initScope A scope to start find.
	 * @param {string}
name A variable name to find.
	 * @returns {eslint-scope.Variable|null}
A found variable or `null`.
	 */
...
,
	 ...
	 * @param {ASTNode}
node A function node to check. It also can be an implicit function, like `StaticBlock`
	 * or any expression that is `PropertyDefinition#value` node.
	 * @param {SourceCode}
sourceCode A SourceCode instance to get comments.
	 * @param {boolean}
[capIsConstructor = true] `false` disables the assumption that functions which name starts
	 * with an uppercase or are assigned to a variable which name starts with an uppercase are constructors.
	 * @returns {boolean}
The function node is the default `this` binding.
	 */
...
node The node to check.
	 * @returns {boolean}
`true` if the node is an empty block.
	 */
	isEmptyBlock(node) {
		return Boolean(
...
node The node to check.
	 * @returns {boolean}
`true` if the node is an empty function.
	 */
...
node The node to check.
	 * @returns {ASTNode[]}
The directives found in the directive prologue.
	 */
...
node The node to check.
	 * @returns {boolean}
`true` if this node is a decimal integer.
	 * @example
	 *
	 * 0         // true
	 * 5         // true
...
token The token to check.
	 * @returns {boolean}
`true` if this token is a decimal integer.
	 */
...
node An expression node
	 * @returns {string}
The text representing the node, with all surrounding parentheses included
	 */
...
,
	 ...
	 * @param {ASTNode}
node ASTNode to check
	 * @returns {boolean}
True if there is a chance it contains an Error obj
	 */
	couldBeError(node) {
		switch (node.type) {
			case "Identifier":
...
node The node to check.
	 * @returns {boolean}
`true` if the node is a number or bigint literal.
	 */
...
,
	 ...
	 * @param {Token|string}
leftValue The left token. If this is a string, it will be tokenized and the last token will be used.
	 * @param {Token|string}
rightValue The right token. If this is a string, it will be tokenized and the first token will be used.
	 * @returns {boolean}
If the tokens cannot be safely placed next to each other, returns `false`. If the tokens can be placed
...
,
	 ...
	 * @param {SourceCode}
sourceCode The source code to convert index to loc.
	 * @param {Comment}
comment The `/*globals` directive comment which include the name.
	 * @param {string}
...
,
	 ...
	 * @param {string}
rawString A string in its raw representation.
	 * @returns {boolean}
`true` if the string contains at least one octal escape sequence
...
node Node to check.
	 * @returns {boolean}
True if the node is a template literal without expressions.
	 */
...
*     else
	 *         bar()
	 * @param {ASTNode}
node `BlockStatement` body with exactly one statement directly inside. The statement can have its own nested statements.
	 * @param {SourceCode}
sourceCode The source code
	 * @returns {boolean}
`true` if the braces are necessary - removing them (replacing the given `BlockStatement` body with its single statement content)
	...
	areBracesNecessary(node, sourceCode) {
		 ...
		 * @param {ASTNode}
nodeToCheck The node to check
		 * @returns {boolean}
True if the node is a lexical declaration
		 * @private
		 */
...
token The token to check.
		 * @returns {boolean}
`true` if the token is an `else` token.
		 */
...
nodeToCheck The node to check.
		 * @returns {boolean}
`true` if the node is followed by an `else` keyword token.
		 */
...
*        else
		 *            bar()
		 * @param {ASTNode}
nodeToCheck Node representing the code to check.
		 * @returns {boolean}
`true` if an `if` statement within the code would become associated with an `else` appended to that code.
		 */
