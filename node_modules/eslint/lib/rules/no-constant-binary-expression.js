 ...
 * @param {Scope}
scope Scope in which the expression was found.
 * @param {ASTNode}
 ...
 * @param {Scope}
scope The scope in which the node was found.
 * @param {ASTNode}
node The AST node being tested.
 * @param {boolean}
nonNullish if `true` then nullish values are not considered constant.
 * @returns {boolean}
Does `node` have constant nullishness?
 */
 ...
 * @returns {boolean}
Is `node` guaranteed to be a boolean?
 */
 ...
 * @param {Scope}
scope The scope in which node was found.
 * @param {ASTNode}
node The node to test.
 * @returns {boolean}
Will `node` always coerce to the same boolean value?
 */
...
case "Literal": // True or false, literals never change
		...
		case "TemplateLiteral":
			 ...
			 * tricky to get right. For example: `000.${foo}
000`
			 */
...
node The node to test
 * @returns {boolean}
Will `node` always give the same result when compared to a
...
scope Scope in which the expression was found.
 * @param {ASTNode}
a One side of the expression
 * @param {ASTNode}
b The other side of the expression
 * @param {string}
operator The binary expression operator
 * @returns {ASTNode | null}
The node which will cause the expression to have a constant result.
 */
...
,
		...
		messages: {
			constantBinaryOperand:
				"Unexpected constant binary expression. Compares constantly with the {{otherSide}
}-hand side of the `{{operator}}`.",
			constantShortCircuit:
				"Unexpected constant {{property}
