...
scope Function scope or a function's inner scope.
		 * @returns {boolean}
True if all names can be safely declared, false otherwise.
		 */
...
const functionScope = scope.variableScope
			...
			if (declaredVariables.some(({ name }
) => names.includes(name))) {
				return false
			}
// Redeclaring a catch variable would also cause a syntax error.
			...
			if (
				usedImplicitVariables.some(({ name }
) => names.includes(name))
			) {
				return false
			}
...
node The 'else' node.
		 * @param {eslint-scope.Scope}
scope The scope in which the node and the whole 'if' statement is.
		 * @returns {boolean}
...
/**
		 ...
		 * @param {Node}
node The consequent/alternate node
		 * @returns {boolean}
True if it has a return
		 */
...
/**
		 ...
		 * @param {Node}
node The consequent node
		 * @returns {boolean}
True if this is a nested rule violation
		 */
...
/**
		 ...
		 * code paths.
		 * @param {Node}
node The consequent or body node
		 * @returns {boolean}
`true` if it is a Return/If node that always returns.
		 */
...
node The node to be checked
		 * @returns {boolean}
`true` if it returns on every codepath.
		 */
