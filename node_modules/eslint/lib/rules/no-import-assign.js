...
node The node to check.
 * @returns {boolean}
`true` if the node is the operand of mutation unary operator.
 */
...
node The node to check.
 * @returns {boolean}
`true` if the node is the iteration variable.
 */
...
node The node to check.
 * @param {Scope}
scope A `escope.Scope` object to find variable (whichever).
 * @returns {boolean}
`true` if the node is at the first argument of a well-known mutation function.
 */
 ...
 * @param {ASTNode}
id The Identifier node to check.
 * @param {Scope}
scope A `escope.Scope` object to find variable (whichever).
 * @returns {boolean}
`true` if the member of `id` was updated.
 */
...
,
		...
		messages: {
			...
			readonlyMember: "The members of '{{name}
}' are read-only.",
		}
,
	}
