 ...
 * @typedef {'array-destructure'|'catch-clause'|'parameter'|'variable'}
VariableType
 */
 ...
 * @typedef {Object}
UnusedVarMessageData
 * @property {string}
varName The name of the unused var.
 * @property {'defined'|'assigned a value'}
...
,
		...
		messages: {
			unusedVar:
				"'{{varName}
}' is {{action}} but never used{{additional}}.",
			usedIgnoredVar:
				"'{{varName}
}' is marked as ignored but is used{{additional}}.",
...
switch (def.type) {
		 ...
		 * @param {VariableType}
variableType a simple name for the types of variables that this rule supports
		 * @throws {Error}
(Unreachable)
...
/**
		 ...
		 * @param {Variable}
unusedVar eslint-scope variable object.
		...
		function getDefinedMessageData(unusedVar) {
			...
			if (def) {
				...
				if (pattern && variableDescription) {
					additionalMessageData = `. Allowed unused ${variableDescription}
must match ${pattern}`
				}
}
...
must match ${pattern}`
		 ...
		 * @param {Variable}
variable eslint-scope variable object
		 * @param {VariableType}
variableType a simple name for the types of variables that this rule supports
		 * @returns {UsedIgnoredVarMessageData}
The message data to be used with
		...
		function getUsedIgnoredMessageData(variable, variableType) {
			...
			if (pattern && variableDescription) {
				additionalMessageData = `. Used ${variableDescription}
...
variable eslint-scope variable object.
		 * @returns {boolean}
True if the variable is declared with "using" or "await using"
		 * @private
		 */
...
variable eslint-scope variable object.
		 * @returns {boolean}
True if the variable has a sibling rest property, false if not.
		 * @private
		 */
...
/**
		 ...
		 * @param {Reference}
ref An eslint-scope Reference
		 * @returns {boolean}
whether the given reference represents a read operation
		 * @private
		 */
...
ref The reference to check.
		 * @param {ASTNode[]}
nodes The candidate function nodes.
		 * @returns {boolean}
...
/**
		 ...
		 * @param {ASTNode}
inner A node which is expected as inside.
		 * @param {ASTNode}
outer A node which is expected as outside.
		 * @returns {boolean}
...
ref A reference to check.
		 * @param {ASTNode}
prevRhsNode The previous RHS node.
...
/**
		 ...
		 * @param {ASTNode}
funcNode A function node to check.
		 * @param {ASTNode}
rhsNode The RHS node of the previous assignment.
		 * @returns {boolean}
...
/**
		 ...
		 * @param {ASTNode}
id An Identifier node to check.
		 * @param {ASTNode}
rhsNode The RHS node of the previous assignment.
		 * @returns {boolean}
...
ref The reference to check.
		 * @returns {boolean}
whether reference is used in the for-in loops
		 * @private
		 */
...
/**
		 ...
		 * @param {eslint-scope.Variable}
variable The variable to check.
		 * @returns {boolean}
`true` if the variable is defined after the last
		 * used parameter.
		 */
...
/**
		 ...
		 * @param {Scope}
scope an eslint-scope Scope object.
		 * @param {Variable[]}
unusedVars an array that saving result.
		 * @returns {Variable[]}
unused variables of the scope and descendant scopes.
		 * @private
		 */
...
fixer fixer object
		 * @param {Object}
unusedVar unused variable to fix
		 * @returns {Object}
fixer object
		 */
		function handleFixes(fixer, unusedVar) {
			 ...
			 * @param {ASTNode}
node node of identifier
			 * @param {number}
skips number of token to skip
			 * @returns {number}
start range of token before the identifier
			 */
...
skips number of token to skip
			 * @returns {number}
end range of token after the identifier
			 */
...
node node of identifier
			 * @returns {string}
value of token before the identifier
			 */
...
node node of identifier
			 * @returns {string}
value of token after the identifier
			 */
...
node ArrayPattern node
			 * @returns {boolean}
true if array has single element with other null elements
			 */
...
node ImportDeclaration node
			 * @param {string}
type type of import to check
			 * @returns {boolean}
true if import specifier has import of specified type
			 */
...
/**
			 ...
			 * @param {ASTNode}
nextToken next token of unused variable
			 * @param {ASTNode}
prevToken previous token of unused variable
			 * @returns {boolean}
true if declaration is not safe to remove
			 */
...
/**
			 ...
			 * @param {ASTNode}
node parent node to identifier
			 * @returns {Object}
fixer object
			 */
...
/*
					 ...
					 * remove complete declaration when there is an unused variable in 'const { a }
= foo;', same for arrays.
					 */
...
/*
					 ...
					 * fix 'let bar = "hello", { a }
= foo;' to 'let bar = "hello";' if 'a' is unused, same for arrays.
					 */
...
/*
					 ...
					 * fix 'let { a }
= foo, bar = "hello";' to 'let  bar = "hello";' if 'a' is unused, same for arrays.
					 */
					return fixer.removeRange([
						parentNode.range[0],
...
// fix const { a:b } = foo
				if (tokenBefore.value === ":") {
					// remove first unused variable in const { a:b }
= foo
					if (
