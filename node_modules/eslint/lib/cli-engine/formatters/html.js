 ...
 * @param {Object}
it data for the document.
 * @returns {string}
HTML document.
 */
function pageTemplate(it) {
	const { reportColor, reportSummary, date, results }
= it
 ...
 * @param {string}
word A word in its singular form.
 * @param {number}
count A number controlling whether word should be pluralized.
 * @returns {string}
...
function renderSummary(totalErrors, totalWarnings) {
	...
	let renderedText = `${totalProblems}
${pluralize("problem", totalProblems)}`

	if (totalProblems !== 0) {
		renderedText += ` (${totalErrors}
${pluralize("error", totalErrors)}, ${totalWarnings} ${pluralize("warning", totalWarnings)})`
	}
return renderedText
}
/**
 ...
 * @param {Object}
it data for the message.
 * @returns {string}
HTML (table row) describing the message.
 */
...
">
    <td>${lineNumber}
:${columnNumber}</td>
    <td class="clr-${severityNumber}
">${severityName}</td>
    <td>${encodeHTML(message)}
</td>
    <td>
        <a href="${ruleUrl ? ruleUrl : ""}
" target="_blank" rel="noopener noreferrer">${ruleId ? ruleId : ""}</a>
    </td>
</tr>
`.trimStart()
}
...
messages Messages.
 * @param {number}
parentIndex Index of the parent HTML row.
 * @param {Object}
rulesMeta Dictionary containing metadata for each rule executed by the analysis.
 * @returns {string}
...
message Message.
	 * @returns {string}
HTML (table row) describing a message.
	 */
	return messages
		.map(message => {
 ...
 * @param {Object}
it data for the file.
 * @returns {string}
HTML (table row) describing the result for the file.
 */
function resultTemplate(it) {
	const { color, index, filePath, summary }
= it
