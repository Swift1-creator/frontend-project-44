...
const { resolve }
= require("../shared/relative-module-resolver.js")
...
/**
 * @import { Config as CalculatedConfig }
from "../config/config.js"
 * @import { FlatConfigArray }
from "../config/flat-config-array.js"
 * @import { RuleDefinition, RulesMeta }
from "@eslint/core"
 * @import { WorkerLintResults }
from "./worker.js"
 */
...
DeprecatedRuleInfo */
/** @typedef {import("../types").ESLint.LintResult}
LintResult */
/** @typedef {import("../types").ESLint.Plugin}
Plugin */
/** @typedef {import("../types").ESLint.ResultsMeta}
ResultsMeta */
 ...
 * @property {Config|Array<Config>}
[baseConfig] Base config, extended by all configs used with this instance
 * @property {boolean}
[cache] Enable result caching.
 * @property {string}
[cacheLocation] The cache file to use instead of .eslintcache.
 * @property {"metadata" | "content"}
[cacheStrategy] The strategy used to detect changed files.
 * @property {number | "auto" | "off"}
[concurrency] Maximum number of linting threads, "auto" to choose automatically, "off" for no multithreading.
 * @property {string}
[cwd] The value to use for the current working directory.
 * @property {boolean}
[errorOnUnmatchedPattern] If `false` then `ESLint#lintFiles()` doesn't throw even if no target files found. Defaults to `true`.
 ...
 * @property {string[]}
[flags] Array of feature flags to enable.
 * @property {boolean}
 ...
 * @property {boolean}
[ignore] False disables all ignore patterns except for the default ones.
 * @property {string[]}
[ignorePatterns] Ignore file patterns to use in addition to config ignores. These patterns are relative to `cwd`.
 * @property {Config|Array<Config>}
[overrideConfig] Override config, overrides all configs used with this instance
 * @property {boolean|string}
[overrideConfigFile] Searches for default config file when falsy
 *      doesn't do any config file lookup when `true`; considered to be a config filename
 *      when a string.
 * @property {boolean}
[passOnNoPatterns=false] When set to true, missing patterns cause
 *      the linting operation to short circuit and not report any failures.
 * @property {Record<string,Plugin>}
[plugins] An array of plugin implementations.
 * @property {boolean}
[stats] True enables added statistics on lint results.
 * @property {boolean}
[warnIgnored] Show warnings when the file list includes ignored files
 */
 ...
 * @param {Map<string, RuleDefinition>}
rules a map of rules from which to generate the object.
 * @returns {Record<string, RulesMeta>}
metadata for all enabled rules.
 */
 ...
 * @param {ESLint}
eslint The ESLint instance.
 * @param {string}
maybeFilePath The absolute path to a lint target file or `"<text>"`.
 * @returns {DeprecatedRuleInfo[]}
The used deprecated rule list.
 */
					...
					retv.push({
						...
						replacedBy: usesNewFormat
							? (meta.deprecated.replacedBy?.map(
									replacement =>
										`${replacement.plugin?.name !== void 0 ? `${getShorthandName(replacement.plugin.name, "eslint-plugin")}
...
eslint The ESLint instance.
 * @param {LintResult[]}
results The linting results to process.
 * @returns {LintResult[]}
The processed linting results.
 */
 ...
 * @param {LintResult}
a The first lint result.
 * @param {LintResult}
b The second lint result.
 * @returns {number}
An integer representing the order in which the two results should occur.
 */
 ...
 * @param {ESLintOptions}
options The ESLint instance options.
...
cause The original error that led to this symptom error being thrown. Might not always be available.
 * @returns {TypeError}
An error object.
 */
 ...
 * @param {number}
processableFileCount The number of files that need to be processed.
 * @param {number}
maxWorkers The maximum number of workers to run.
 * @returns {number}
 ...
 * @param {LintResultCache}
lintResultCache The lint result cache.
 * @param {boolean}
fix The fix option.
 * @param {string}
filePath The file for which to retrieve lint results.
 * @param {Config}
config The config of the file.
 * @returns {boolean}
True if the file needs to be reprocessed.
 */
 ...
 * @param {ESLint}
eslint ESLint instance.
 * @param {string[]}
filePaths File paths to lint.
 * @param {number}
maxWorkers The maximum number of workers to run.
 * @returns {number}
The number of worker threads to run for "auto" concurrency.
 */
 ...
 * @param {string[]}
filePaths File paths to lint.
 * @param {{ availableParallelism: () => number }
} [os] Node.js `os` module, or a mock for testing.
 * @returns {number}
The effective number of worker threads to be started. A value of zero disables multithread linting.
 */
...
= os,
	...
	const { concurrency }
= privateMembers.get(eslint).options
	switch (concurrency) {
		case "off":
			return 0
		case "auto": {
...
filePaths File paths to lint.
 * @param {number}
workerCount The number of worker threads to run.
 * @param {ESLintOptions | string}
eslintOptionsOrURL The unprocessed ESLint options or the URL of the options module.
 * @param {() => void}
warnOnLowNetLintingRatio A function to call if the net linting ratio is low.
...
let worstNetLintingRatio = 1
	 ...
	 * @param {() => void}
resolve_ Called when the worker thread terminates successfully.
	 * @param {(error: Error) => void}
reject Called when the worker thread terminates with an error.
	 * @returns {void}
*/
...
async function lintFilesWithMultithreading(
	...
	const { configLoader, lintResultCache }
= privateMembers.get(eslint)
...
return false
	...
	const error = new TypeError(
		`The ${uncloneableOptionKeys.length === 1 ? "option" : "options"}
${new Intl.ListFormat("en-US").format(uncloneableOptionKeys.map(key => `"${key}"`))} cannot be cloned. When concurrency is enabled, all options must be cloneable values (JSON values). Remove uncloneable options or use an options module.`,
	)
...
/**
	 ...
	 * @param {URL}
optionsURL The URL of the options module.
	 * @returns {ESLint}
The new ESLint instance.
	 */
...
/**
	 ...
	 * @param {LintResult[]}
results The results to fetch rules meta for.
	 * @returns {Record<string, RulesMeta>}
...
*/
		...
		if (workerCount) {
			debug(`Linting using ${workerCount}
worker thread(s).`)
...
/**
	 ...
	 * @param {string}
code A string of JavaScript code to lint.
	 * @param {Object}
[options] The options.
	 * @param {string}
[options.filePath] The path to the file of the source code.
	 * @param {boolean}
[options.warnIgnored] When set to true, warn if given filePath is an ignored path.
	 * @returns {Promise<LintResult[]>}
...
// Options validation
		...
		const { filePath, warnIgnored, ...unknownOptions }
= options || {}
...
// Now we can get down to linting
		...
		const { linter, options: eslintOptions }
= privateMembers.get(this)
		const {
			allowInlineConfig,
			cwd,
...
/**
	 ...
	 * @param {string}
[name] The name of the formatter to load.
...
.js`,
		...
		try {
			...
			if (removedFormatters.has(name)) {
				ex.message = `The ${name}
formatter is no longer part of core ESLint. Install it manually with \`npm install -D eslint-formatter-${name}\``
			}
else {
...
results The lint results to format.
			 * @param {ResultsMeta}
resultsMeta Warning count and max threshold.
			 * @returns {string}
The formatted lint results.
			 */
...
filePath The path of the file to retrieve a config object for.
	 * @returns {Promise<CalculatedConfig|undefined>}
...
/**
	 ...
	 * @param {string}
[filePath] The path of the file to find the config file for.
	 * @returns {Promise<string|undefined>}
The path to the config file being used or
 ...
 * @returns {Promise<boolean>}
Whether flat config should be used.
 */
