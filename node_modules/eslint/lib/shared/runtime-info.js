 ...
 * @returns {string}
A string that contains execution environment information.
 */
function environment() {
	 ...
	 * @param {string}
parentPath The parent path to check.
	 * @param {string}
childPath The path to check.
	 * @returns {boolean}
Whether or not the given path is a child of a directory.
	 */
...
cmd The command to execute.
	 * @param {Array}
args The arguments to be executed with the command.
	 * @throws {Error}
As may be collected by `cross-spawn.sync`.
	 * @returns {string}
...
/**
	 ...
	 * @param {string}
versionStr The string to normalize.
	 * @returns {string}
...
As may be collected by `cross-spawn.sync`.
	 * @returns {string}
The normalized version returned by the command.
	 */
	function getBinVersion(bin) {
		...
		try {
			...
			log.error(
				`Error finding ${bin}
version running the command \`${bin} ${binArgs.join(" ")}\``,
			)
			throw e
		}
}
...
pkg The package to check.
	 * @param {boolean}
global Whether to check globally or not.
	 * @throws {Error}
As may be collected by `cross-spawn.sync`.
	 * @returns {string}
The normalized version returned by the command.
...
return normalizeVersionStr(pkgVersion)
			...
			log.error(
				`Error finding ${pkg}
version running the command \`npm ${npmLsArgs.join(" ")}\``,
			)
			throw e
		}
}
 ...
 * @returns {string}
The version from the currently executing ESLint's package.json.
 */
function version() {
