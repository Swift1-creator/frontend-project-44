...
LintMessage */
/** @typedef {import("../types").Linter.LintSuggestion}
SuggestionResult */
/** @typedef {import("@eslint/core").Language}
Language */
/** @typedef {import("@eslint/core").SourceLocation}
SourceLocation */
...
MessageDescriptor
 * @property {ASTNode}
[node] The reported node
 * @property {Location}
loc The location of the problem.
 * @property {string}
message The problem message.
 * @property {Object}
[data] Optional data to use to fill in placeholders in the
 *      message.
 * @property {Function}
[fix] The function to call that creates a fix command.
 * @property {Array<{desc?: string, messageId?: string, fix: Function}
>} suggest Suggestion descriptions and functions to create a the associated fixes.
 */
...
LintProblem
 * @property {string}
ruleId The rule ID that reported the problem.
 * @property {string}
message The problem message.
 ...
 * @param {number}
location.line The starting line number.
 * @param {number}
location.column The starting column number.
 * @param {number}
[location.endLine] The ending line number.
 * @param {number}
[location.endColumn] The ending column number.
 * @param {Language}
...
function createMissingRuleMessage(ruleId) {
	return Object.hasOwn(ruleReplacements.rules, ruleId)
		? `Rule '${ruleId}
' was removed and replaced by: ${ruleReplacements.rules[ruleId].join(", ")}`
		: `Definition for rule '${ruleId}
' was not found.`
}
/**
 * creates a linting problem
 * @param {LintProblem}
options to create linting error
 * @param {RuleSeverity}
severity the error message to report
 * @param {Language}
language the language to use to adjust the location information.
 * @returns {LintMessage}
created problem, returns a missing-rule problem if only provided ruleId.
 * @private
 */
 ...
 * @param {...*}
args A list of arguments passed to `context.report`
 * @returns {MessageDescriptor}
A normalized object containing report information
 */
...
descriptor A descriptor to validate
 * @returns {void}
 ...
 * @param {MessageDescriptor}
descriptor A descriptor for the report from a rule.
 * @returns {{start: Location, end: (Location|null)}
} An updated location that infers the `start` and `end` properties
 ...
 * @param {Fix|null}
fix The fix to clone.
 * @returns {Fix|null}
Deep cloned fix object or `null` if `null` or `undefined` was passed in.
 */
 ...
 * @param {Fix[]}
fixes The fixes to merge.
 * @param {SourceCode}
sourceCode The source code object to get the text between fixes.
 ...
 * @param {MessageDescriptor}
descriptor The report descriptor.
 * @param {SourceCode}
sourceCode The source code object to get text between fixes.
 * @returns {({text: string, range: number[]}
|null)} The fix for the descriptor
 */
...
sourceCode The source code object to get text between fixes.
 * @param {Object}
messages Object of meta messages for the rule.
 * @returns {Array<SuggestionResult>}
The suggestions for the descriptor
 */
 ...
 * @param {string}
[options.messageId] The error message ID.
 * @param {{start: SourceLocation, end: (SourceLocation|null)}
} options.loc Start and end location
 * @param {{text: string, range: (number[]|null)}
} options.fix The fix object
 * @param {Array<{text: string, range: (number[]|null)}
>} options.suggestions The array of suggestions objects
 * @param {Language}
[options.language] The language to use to adjust line and column offsets.
 * @returns {LintMessage}
 ...
 * @param {Array<{ desc?: string, messageId?: string }
>} suggest The incoming suggest data.
 * @param {Object}
messages Object of meta messages for the rule.
 * @returns {void}
*/
...
= suggestion
				...
				if (!messages) {
					throw new TypeError(
						`context.report() called with a suggest option with a messageId '${messageId}
', but no messages were present in the rule metadata.`,
					)
				}
if (!messages[messageId]) {
					throw new TypeError(
						`context.report() called with a suggest option with a messageId '${messageId}
' which is not present in the 'messages' config: ${JSON.stringify(messages, null, 2)}`,
					)
				}
if (suggestion.desc) {
					throw new TypeError(
 ...
 * @throws {TypeError}
If messageId is not found or both message and messageId are provided.
 */
...
if (!messages || !Object.hasOwn(messages, id)) {
			throw new TypeError(
				`context.report() called with a messageId of '${id}
' which is not present in the 'messages' config: ${JSON.stringify(messages, null, 2)}`,
			)
		}
return messages[id]
...
*/
	 ...
	 * @param {Object}
options The options for the file report
	 * @param {string => RuleDefinition}
options.ruleMapper A rule mapper that maps rule IDs to their metadata.
	 * @param {SourceCode}
options.sourceCode The source code object for the file.
	 * @param {Language}
options.language The language to use to adjust line and column offsets.
	 * @param {boolean}
[options.disableFixes=false] Whether to disable fixes for this report.
	 */
...
ruleId The rule ID that reported the problem.
	 * @param {0|1|2}
severity The severity of the problem (0 = off, 1 = warning, 2 = error).
	 * @param {...*}
args The arguments passed to `context.report()`.
	 * @returns {LintMessage}
The created message object.
	 * @throws {TypeError}
If the messageId is not found or both message and messageId are provided.
	 * @throws {AssertionError}
If the node is not an object or neither a node nor a loc is provided.
	 */
...
/**
	 ...
	 * @param {LintProblem}
descriptor The descriptor for the error message.
	 * @returns {LintMessage}
The created message object.
	 */
	addError(descriptor) {
...
/**
	 ...
	 * @param {LintProblem}
descriptor The descriptor for the fatal error message.
	 * @returns {LintMessage}
The created message object.
	 */
	addFatal(descriptor) {
...
/**
	 ...
	 * @param {LintProblem}
descriptor The descriptor for the warning message.
	 * @returns {LintMessage}
The created message object.
	 */
