...
/**
 * @typedef {import("esquery").Selector}
ESQuerySelector
 * @typedef {import("esquery").ESQueryOptions}
ESQueryOptions
 */
...
*/
	 ...
	 * @param {string}
source The raw selector string that was parsed
	 * @param {boolean}
isExit Whether this selector is an exit selector
	 * @param {ESQuerySelector}
root An object (from esquery) describing the matching behavior of the selector
	 * @param {string[]|null}
nodeTypes The node types that could possibly trigger this selector, or `null` if all node types could trigger it
	 * @param {number}
attributeCount The number of class, pseudo-class, and attribute queries in this selector
	 * @param {number}
identifierCount The number of identifier queries in this selector
	 */
	constructor(
		source,
		isExit,
...
/**
	 ...
	 * @param {ESQueryParsedSelector}
otherSelector The selector to compare against
	 * @returns {number}
 ...
 * @returns {any[]}
The intersection of the input arrays
 */
 ...
 * @param {ESQuerySelector}
parsedSelector An object (from esquery) describing the matching behavior of the selector
 * @returns {{nodeTypes:string[]|null, attributeCount:number, identifierCount:number}
} Object containing selector data.
 */
 ...
 * @returns {Object|null}
An object describing the selector if it is simple, or `null` if it is not.
 */
...
selector The selector string to parse.
 * @returns {Object}
An object (from esquery) describing the matching behavior of this selector
 * @throws {Error}
	...
	try {
		...
		if (
			...
			throw new SyntaxError(
				`Syntax error in selector "${selector}
" at position ${err.location.start.offset}: ${err.message}`,
				{
					cause: err,
				}
,
 ...
 * @param {string}
source A raw AST selector
 * @returns {ESQueryParsedSelector}
A selector descriptor
 */
 ...
 * @param {Object}
node The node to check against the selector.
 * @param {ESQuerySelector}
root The root of the selector to match against.
 * @param {Object[]}
ancestry The ancestry of the node being checked, which is an array of nodes from the current node to the root.
 * @param {ESQueryOptions}
options The options to use for matching.
 * @returns {boolean}
`true` if the node matches the selector, `false` otherwise.
 */
