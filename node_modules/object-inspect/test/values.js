...
test('Map', { skip: typeof Map !== 'function' }
, function (t) {
    var map = new Map()
    map.set({ a: 1 }
, ['b'])
    map.set(3, NaN)
    var expectedString = 'Map (2) {' + inspect({ a: 1 }
) + ' => ' + inspect(['b']) + ', 3 => NaN}'
    t.equal(inspect(map), expectedString, 'new Map([[{ a: 1 }
, ["b"]], [3, NaN]]) should show size and contents')
    t.equal(inspect(new Map()), 'Map (0) {}
', 'empty Map should show as empty')

    var nestedMap = new Map()
    nestedMap.set(nestedMap, map)
    t.equal(inspect(nestedMap), 'Map (1) {[Circular] => ' + expectedString + '}
...
test('WeakMap', { skip: typeof WeakMap !== 'function' }
, function (t) {
    var map = new WeakMap()
    map.set({ a: 1 }
, ['b'])
...
'
    t.equal(inspect(map), expectedString, 'new WeakMap([[{ a: 1 }
, ["b"]]]) should not show size or contents')
    t.equal(inspect(new WeakMap()), 'WeakMap { ? }
...
test('Set', { skip: typeof Set !== 'function' }
, function (t) {
    var set = new Set()
    set.add({ a: 1 }
)
    set.add(['b'])
    var expectedString = 'Set (2) {' + inspect({ a: 1 }
) + ', ' + inspect(['b']) + '}'
    t.equal(inspect(set), expectedString, 'new Set([{ a: 1 }
, ["b"]]) should show size and contents')
    t.equal(inspect(new Set()), 'Set (0) {}
', 'empty Set should show as empty')

    var nestedSet = new Set()
    nestedSet.add(set)
    nestedSet.add(nestedSet)
    t.equal(inspect(nestedSet), 'Set (2) {' + expectedString + ', [Circular]}
...
test('WeakSet', { skip: typeof WeakSet !== 'function' }
, function (t) {
    var map = new WeakSet()
    map.add({ a: 1 }
)
...
'
    t.equal(inspect(map), expectedString, 'new WeakSet([{ a: 1 }
]) should not show size or contents')
    t.equal(inspect(new WeakSet()), 'WeakSet { ? }
...
test('WeakRef', { skip: typeof WeakRef !== 'function' }
, function (t) {
    var ref = new WeakRef({ a: 1 }
)
    var expectedString = 'WeakRef { ? }
'
    t.equal(inspect(ref), expectedString, 'new WeakRef({ a: 1 }
...
test('FinalizationRegistry', { skip: typeof FinalizationRegistry !== 'function' }
, function (t) {
    var registry = new FinalizationRegistry(function () {}
)
    var expectedString = 'FinalizationRegistry [FinalizationRegistry] {}
'
    t.equal(inspect(registry), expectedString, 'new FinalizationRegistry(function () {}
) should work normallys')
...
test('Strings', function (t) {
    ...
    t.equal(inspect(str, { quoteStyle: 'single' }
), "'" + str + "'", 'primitive string shows as such, single quoted')
    t.equal(inspect(str, { quoteStyle: 'double' }
    ...
    t.equal(inspect(Object(str), { quoteStyle: 'single' }
), 'Object(' + inspect(str, { quoteStyle: 'single' }) + ')', 'String object shows as such, single quoted')
    t.equal(inspect(Object(str), { quoteStyle: 'double' }
), 'Object(' + inspect(str, { quoteStyle: 'double' }) + ')', 'String object shows as such, double quoted')
...
test('Proxies', { skip: typeof Proxy !== 'function' || !hasToStringTag }
, function (t) {
...
test('fakers', { skip: !hasToStringTag }
, function (t) {
