...
var groups = function groups(matchObject) {
	return hasGroups ? assign(matchObject, { groups: matchObject.groups }
...
var testResults = function (t, iterator, expectedResults, item) {
	...
	t.test(prefix + 'actual vs expected results', { skip: results.length !== expecteds.length }
, function (st) {
		forEach(expecteds, function (expected, index) {
			var result = results.shift()
			st.equal(result.done, expected.done, 'result ' + (index + 1) + ' is ' + (expected.done ? '' : 'not ') + 'done')
			st.test('result ' + (index + 1), { skip: result.done !== expected.done }
, function (s2t) {
				if (expected.done) {
					...
					s2t.deepEqual(entries(result.value || {}
), entries(expected.value || {}), 'result ' + (index + 1) + ' has the same entries')
...
module.exports = function (matchAll, regexMatchAll, t) {
	t.test('non-regexes', function (st) {
		var notRegexes = [
			...
			[undefined, [
				{ value: assign([''], groups({ index: 0, input: 'abc' }
)), done: false },
				{ value: assign([''], groups({ index: 1, input: 'abc' }
)), done: false },
				{ value: assign([''], groups({ index: 2, input: 'abc' }
)), done: false },
				{ value: assign([''], groups({ index: 3, input: 'abc' }
)), done: false },
			...
			[{}
, [
				{ value: assign(['b'], groups({ index: 1, input: 'abc' }
)), done: false },
				{ value: assign(['c'], groups({ index: 2, input: 'abc' }
)), done: false },
				{ value: undefined, done: true }
]],
			[[], [
	...
	t.test('ToString-able objects', function (st) {
		...
		var regex = /[ac]/g
		var expectedResults = [
			{ value: assign(['a'], groups({ index: 0, input: str }
)), done: false },
			{ value: assign(['a'], groups({ index: 1, input: str }
)), done: false },
			{ value: assign(['c'], groups({ index: 3, input: str }
)), done: false },
			{ value: undefined, done: true }
]
...
,
		...
		st.test('with a static flags property', function (s2t) {
			var str = 'AaBC'
			var regex = /[ac]/
			define(regex, { flags: 'ig' }
, { flags: function () { return true; } })
			try {
				define(regex, { global: true }
...
s2t.equal(regex.flags, 'ig')
		...
		st.test('respects flags', function (s2t) {
			...
			var expectedResults = [
				...
				{ value: assign(['a'], groups({ index: 2, input: str }
)), done: false },
				{ value: assign(['C'], groups({ index: 6, input: str }
)), done: false },
				{ value: undefined, done: true }
]
			testResults(s2t, matchAll(str, regex), expectedResults)
...
,
		...
		st.test('works with a global non-sticky regex', function (s2t) {
			...
			var regex = /[bc]/gi
			var expectedResults = [
				{ value: assign(['B'], groups({ index: 2, input: str }
)), done: false },
				{ value: assign(['b'], groups({ index: 3, input: str }
)), done: false },
				{ value: assign(['C'], groups({ index: 4, input: str }
)), done: false },
				{ value: assign(['c'], groups({ index: 5, input: str }
)), done: false },
				{ value: undefined, done: true }
]
			testResults(s2t, matchAll(str, regex), expectedResults)
...
var expectedResults = [
	...
	t.test('zero-width matches', function (st) {
		...
		st.test('global', function (s2t) {
			var expectedResults = [
				{ value: assign([''], groups({ index: 1, input: str }
)), done: false },
				{ value: assign([''], groups({ index: 2, input: str }
)), done: false },
				{ value: assign([''], groups({ index: 3, input: str }
)), done: false },
				{ value: assign([''], groups({ index: 4, input: str }
)), done: false },
				{ value: undefined, done: true }
]
