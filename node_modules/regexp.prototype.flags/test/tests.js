...
var sortedFlags = availableFlags.slice().sort(function (a, b) { return a.localeCompare(b); }
).join('')
...
var randomFlags = availableFlags.slice().sort(function () { return Math.random() > 0.5 ? 1 : -1; }).join('').replace('v', '')
	...
	t.test('generic flags', function (st) {
		...
		st.equal(flags({ ignoreCase: true }
), 'i')
		st.equal(flags({ dotAll: 1, global: 0, sticky: 1, unicode: 1 }
), 'suy')
		st.equal(flags({ __proto__: { multiline: true }
}), 'm')
...
forEach(availableFlags, function (flag) {
	...
	t.test('getters', { skip: !supportsDescriptors }
, function (st) {
