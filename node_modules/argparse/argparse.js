...
_expand_help(action) {
        let params = Object.assign({ prog: this._prog }
, action)
...
// register actions
        ...
        ;[ 'storeConst', 'storeTrue', 'storeFalse', 'appendConst' ].forEach(old_name => {
            ...
            this.register('action', old_name, util.deprecate(this._registry_get('action', new_name),
                sub('{action: "%s"}
is renamed to {action: "%s"}', old_name, new_name)))
        }
)
        // end
    ...
    add_argument() {
        ...
        if (args.length === 1 && Array.isArray(args[0])) {
            ...
            deprecate('argument-array',
                sub('use add_argument(%(args)s, {...}
) instead of add_argument([ %(args)s ], { ... })', {
...
const _ArgumentGroup = _callable(class _ArgumentGroup extends _ActionsContainer {
    ...
    constructor() {
        ...
        super(Object.assign({ description }
, kwargs))
...
return result
        ...
        this.register('type', 'string',
            util.deprecate(String, 'use {type:"str"}
or {type:String} instead of {type:"string"}'))
        // end
...
// create the parsers action and add it to the positionals list
        ...
        // eslint-disable-next-line new-cap
        let action = new parsers_class(Object.assign({ option_strings: [] }
, kwargs))
...
Object.entries({ ONE_OR_MORE, OPTIONAL, PARSER, REMAINDER, SUPPRESS, ZERO_OR_MORE }).forEach(([ n, v ]) => {
        ...
        Object.entries({ _UNRECOGNIZED_ARGS_ATTR }
).forEach(([ n, v ]) => {
            Object.defineProperty(result, n, {
                get() {
