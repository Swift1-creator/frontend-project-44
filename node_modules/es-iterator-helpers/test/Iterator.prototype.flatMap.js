...
t['throws'](
		...
		forEach(v.nonFunctions, function (nonFunction) {
			t['throws'](
				function () { flatMap({ next: function () {}
}, nonFunction); },
				TypeError,
		...
		t.test('actual iteration', { skip: !hasSymbols }
, function (st) {
			var arr = [1, 2, 3]
			var iterator = callBind(arr[Symbol.iterator], arr)

			st['throws'](
				function () { return new flatMap(iterator()); }
, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			)
			st['throws'](
				function () { return new flatMap(iterator(), function () {}
); }, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			)
...
,
			...
			forEach(v.strings, function (string) {
				st['throws'](
					function () { flatMap(iterator(), function () { return string; }
).next(); },
					TypeError,
			...
			testIterator(flatMap(iterator(), function (x) { return [x][Symbol.iterator](); }
), [1, 2, 3], st, 'identity mapper in array iterator')
			testIterator(flatMap(iterator(), function (x) { return [2 * x][Symbol.iterator](); }
), [2, 4, 6], st, 'doubler mapper in array iterator')

			testIterator(flatMap(iterator(), function () { return []; }
), [], st, 'empty mapper in nested array iterator')
			testIterator(flatMap(iterator(), function (x) { return [[x, x + 1]][Symbol.iterator](); }
), [[1, 2], [2, 3], [3, 4]], st, 'identity mapper in nested array iterator')
			testIterator(flatMap(iterator(), function (x) { return [[2 * x, 2 * (x + 1)]][Symbol.iterator](); }
), [[2, 4], [4, 6], [6, 8]], st, 'doubler mapper in nested array iterator')
...
,
	shimmed: function () {
		test('Iterator.prototype.' + fnName + ': shimmed', function (t) {
			t.test('Function name', { skip: !functionsHaveNames }
, function (st) {
			...
			t.test('enumerability', { skip: !defineProperties.supportsDescriptors }
, function (et) {
			...
			t.test('bad string/this value', { skip: !hasStrictMode }
, function (st) {
				st['throws'](function () { return Iterator.prototype[fnName].call(undefined, 'a'); }
, TypeError, 'undefined is not an object')
				st['throws'](function () { return Iterator.prototype[fnName].call(null, 'a'); }
