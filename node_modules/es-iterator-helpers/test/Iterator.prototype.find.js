...
, function (st) {
			...
			st['throws'](
				function () { return new find(iterator()); }
, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			)
			st['throws'](
				function () { return new find(iterator(), function () {}
); }, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			)
			...
			st.equal(find(iterator(), function () { return false; }
), undefined, 'find for always-false')
			st.equal(find(iterator(), function () { return true; }
), 1, 'find for always-true')
			st.equal(find(iterator(), function (x, i) { return x === 2 && i === 1; }
), 2, 'find returns value for matching value/index')
