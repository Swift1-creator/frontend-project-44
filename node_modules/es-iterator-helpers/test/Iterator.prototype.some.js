...
, function (st) {
			...
			st['throws'](
				function () { return new some(iterator()); }
, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			)
			st['throws'](
				function () { return new some(iterator(), function () {}
); }, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			)
			...
			st.equal(some(iterator(), function () { return false; }
), false, 'some for always-false')
			st.equal(some(iterator(), function () { return true; }
), true, 'some for always-true')
			st.equal(some(iterator(), function (x, i) { return x === 2 && i === 1; }
), true, 'some returns true for matching value/index')
