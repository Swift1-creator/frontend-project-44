...
,
		...
		forEach(v.objects, function (nonIterator) {
			t.doesNotThrow(function () { zipKeyed({ a: nonIterator }
); }, 'does not throw until `.next()`')

			t['throws'](
				function () { zipKeyed({ a: nonIterator }
).next(); },
				TypeError,
...
badIterable[Symbol.iterator] = nonFunction
					st['throws'](
						function () { zipKeyed({ a: [], b: badIterable, c: [] }
).next(); },
						TypeError,
