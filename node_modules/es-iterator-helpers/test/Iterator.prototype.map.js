...
, function (st) {
			...
			st['throws'](
				function () { return new map(iterator()); }
, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			)
			st['throws'](
				function () { return new map(iterator(), function () {}
); }, // eslint-disable-line new-cap
				TypeError,
				'`' + name + '` iterator is not a constructor'
			)
			...
			testIterator(map(iterator(), function (x) { return x; }
), [1, 2, 3], st, 'identity mapper')
			testIterator(map(iterator(), function (x) { return 2 * x; }
), [2, 4, 6], st, 'doubler mapper')

			st.test('generators', { skip: generators.length === 0 }
, function (s2t) {
