...
msg(s, type = typescript_exports.server.Msg.Err) {
      ...
      s = `[${typescript_exports.server.nowString()}
] ${s}
`
...
returned: ${e.message}`)
        ...
        logger.info(`watchDirectory for ${path}
uses cached drive information.`)
      }
if (status) {
...
if (this.logger.loggingEnabled() && this.logger.getLogFileName()) {
        args.push(typescript_exports.server.Arguments.LogFile, (0, typescript_exports.combinePaths)((0, typescript_exports.getDirectoryPath)((0, typescript_exports.normalizeSlashes)(this.logger.getLogFileName())), `ti-${process.pid}
.log`))
      }
...
=${currentPort + 1}`)
      ...
      this.host.setImmediate(() => this.event({ pid: this.installer.pid }
, "typingsInstallerPid"))
      process.on("exit", () => {
        this.installer.kill()
      }
      ...
      if (this.canUseEvents && this.eventPort) {
        if (!this.eventSocket) {
          if (this.logger.hasLevel(typescript_exports.server.LogLevel.verbose)) {
            this.logger.info(`eventPort: event "${eventName}
" queued, but socket not yet initialized`)
          }
...
:${typescript_exports.server.indent(json)}`)
      }
process.send(msg)
    }
...
const usersDir = platformIsDarwin ? "Users" : "home"
    const homePath = import_os.default.homedir && import_os.default.homedir() || process.env.HOME || (process.env.LOGNAME || process.env.USER) && `/${usersDir}
/${process.env.LOGNAME || process.env.USER}` || import_os.default.tmpdir()
...
,
      resetRequest(requestId) {
        if (currentRequestId !== requestId) {
          throw new Error(`Mismatched request id, expected ${currentRequestId}
, actual ${requestId}`)
        }
...
NodeVersion: ${process.version} CaseSensitive: ${typescript_exports.sys.useCaseSensitiveFileNames}`)
  logger.info(`ServerMode: ${serverMode}
hasUnknownServerMode: ${unknownServerMode}`)
