...
this.writeLine = text => {
      ...
      try {
        fs.appendFileSync(this.logFile, `[${typescript_exports.server.nowString()}
] ${text}${typescript_exports.sys.newLine}`)
      }
catch {
        this.logFile = void 0
      }
...
function loadTypesRegistryFile(typesRegistryFilePath, host, log2) {
  if (!host.fileExists(typesRegistryFilePath)) {
    if (log2.isEnabled()) {
      log2.writeLine(`Types registry file '${typesRegistryFilePath}
' does not exist`)
    }
...
try {
    ...
    if (log2.isEnabled()) {
      log2.writeLine(`Error when loading types registry file '${typesRegistryFilePath}
': ${e.message}, ${e.stack}`)
    }
return /* @__PURE__ */ new Map()
  }
...
function getTypesRegistryFileLocation(globalTypingsCacheLocation2) {
  return (0, typescript_exports.combinePaths)((0, typescript_exports.normalizeSlashes)(globalTypingsCacheLocation2), `node_modules/${typesRegistryPackageName}
/index.json`)
}
...
`)
      this.log.writeLine(`NPM location: ${this.npmPath}
(explicit '${typescript_exports.server.Arguments.NpmLocation}' ${npmLocation2 === void 0 ? "not " : ""} provided)`)
...
this.ensurePackageDirectoryExists(globalTypingsCacheLocation2)
    try {
      if (this.log.isEnabled()) {
        this.log.writeLine(`Updating ${typesRegistryPackageName}
npm package...`)
      }
this.execSyncAndLog(`${this.npmPath} install --ignore-scripts ${typesRegistryPackageName}@${this.latestDistTag}`, { cwd: globalTypingsCacheLocation2 })
      if (this.log.isEnabled()) {
        this.log.writeLine(`Updated ${typesRegistryPackageName}
npm package`)
      }
} catch (e) {
      if (this.log.isEnabled()) {
        this.log.writeLine(`Error updating ${typesRegistryPackageName}
package: ${e.message}`)
      }
...
process.send(response)
  ...
  installWorker(requestId, packageNames, cwd, onRequestCompleted) {
    if (this.log.isEnabled()) {
      this.log.writeLine(`#${requestId}
with cwd: ${cwd} arguments: ${JSON.stringify(packageNames)}`)
    }
const start = Date.now()
    ...
    if (this.log.isEnabled()) {
      this.log.writeLine(`npm install #${requestId}
...
return false
      ...
      const { stdout, stderr }
= error
      this.log.writeLine(`    Failed. stdout:${indent(typescript_exports.sys.newLine, stdout)}
${typescript_exports.sys.newLine}    stderr:${indent(typescript_exports.sys.newLine, stderr)}`)
      return true
    }
}
}
...
if (log.isEnabled()) {
  process.on("uncaughtException", e => {
    log.writeLine(`Unhandled exception: ${e}
at ${e.stack}`)
  }
)
}
