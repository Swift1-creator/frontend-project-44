...
const valueVariableName = valueVariable
        ...
        const expectedSetterVariableNames = upperCaseCandidatePrefix ? [
          `set${upperCaseCandidatePrefix.charAt(0).toUpperCase()}
${upperCaseCandidatePrefix.slice(1)}${caseCandidateSuffix}`,
          `set${upperCaseCandidatePrefix.toUpperCase()}
${caseCandidateSuffix}`,
        ] : []
...
, ${expectedSetterVariableNames[0]}]`
          ...
          if (isSingleGetter && isUseStateCalledWithSingleArgument) {
            ...
            if (useMemoReactImportSpecifier) {
              ...
              useMemoCode = `${defaultReactImportName}
.useMemo`
            }
else {
              useMemoCode = 'useMemo'
              ...
              getMessageData('suggestMemo', messages.suggestMemo),
              {
                fix: fixer => [
                  ...
                  fixer.replaceTextRange(
                    node.range,
                    `${useMemoCode}
(() => ${getText(context, node.arguments[0])}, [])`
                  ),
                ].filter(Boolean),
              }
))
