 ...
 *
 * @param {Object}
childNode - JSX element being tested.
 * @returns {boolean}
Whether or not the node name match the JSX tag convention.
 */
 ...
 *
 * @param {string}
name - Attribute name to be normalized
 * @returns {string}
Result
 */
 ...
 * @param {JSXAttribute}
node - JSXAttribute being tested.
 * @returns {string | null}
tag name
 */
...
The standard name of the attribute, or undefined if no standard name was found.
 */
function getStandardName(name, context) {
  if (has(DOM_ATTRIBUTE_NAMES, name)) {
    return DOM_ATTRIBUTE_NAMES[/** @type {keyof DOM_ATTRIBUTE_NAMES}
*/ (name)]
  }
if (has(SVGDOM_ATTRIBUTE_NAMES, name)) {
    return SVGDOM_ATTRIBUTE_NAMES[/** @type {keyof SVGDOM_ATTRIBUTE_NAMES}
*/ (name)]
  }
...
const messages = {
  invalidPropOnTag: 'Invalid property \'{{name}
}\' found on tag \'{{tagName}}\', but it is only allowed on: {{allowedTags}}',
  unknownPropWithStandardName: 'Unknown property \'{{name}
}\' found, use \'{{standardName}}\' instead',
  unknownProp: 'Unknown property \'{{name}
}\' found',
  dataLowercaseRequired: 'React does not recognize data-* props with uppercase characters on a DOM element. Found \'{{name}
}\', use \'{{lowerCaseName}}\' instead',
}
...
// Let's dive deeper into tags that are HTML/DOM elements (`<button>`), and not React components (`<Button />`)
        ...
        const allowedTags = has(ATTRIBUTE_TAGS_MAP, name)
          ? ATTRIBUTE_TAGS_MAP[/** @type {keyof ATTRIBUTE_TAGS_MAP}
*/ (name)]
          : null
