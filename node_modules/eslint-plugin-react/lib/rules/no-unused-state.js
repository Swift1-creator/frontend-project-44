 ...
 *
 * State field definitions are collected from `this.state = {}
` assignments in
 * the constructor, objects passed to `this.setState()`, and `state = {}
` class
...
const unwrappedNode = astUtil.unwrapTSAsExpression(node)
        ...
        // If we're looking at a `this.setState({}
)` invocation, record all the
