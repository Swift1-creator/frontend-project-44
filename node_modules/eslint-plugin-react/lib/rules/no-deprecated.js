...
// 0.12.0
  deprecated[`${pragma}
.renderComponent`] = ['0.12.0', `${pragma}.render`]
  deprecated[`${pragma}
.renderComponentToString`] = ['0.12.0', `${pragma}.renderToString`]
  deprecated[`${pragma}
.renderComponentToStaticMarkup`] = ['0.12.0', `${pragma}.renderToStaticMarkup`]
  deprecated[`${pragma}
.isValidComponent`] = ['0.12.0', `${pragma}.isValidElement`]
  deprecated[`${pragma}
.PropTypes.component`] = ['0.12.0', `${pragma}.PropTypes.element`]
  deprecated[`${pragma}
.PropTypes.renderable`] = ['0.12.0', `${pragma}.PropTypes.node`]
  deprecated[`${pragma}
.isValidClass`] = ['0.12.0']
  ...
  // 0.13.0
  deprecated[`${pragma}
.addons.classSet`] = ['0.13.0', 'the npm module classnames']
  deprecated[`${pragma}
.addons.cloneWithProps`] = ['0.13.0', `${pragma}.cloneElement`]
  // 0.14.0
  deprecated[`${pragma}
.render`] = ['0.14.0', 'ReactDOM.render']
  deprecated[`${pragma}
.unmountComponentAtNode`] = ['0.14.0', 'ReactDOM.unmountComponentAtNode']
  deprecated[`${pragma}
.findDOMNode`] = ['0.14.0', 'ReactDOM.findDOMNode']
  deprecated[`${pragma}
.renderToString`] = ['0.14.0', 'ReactDOMServer.renderToString']
  deprecated[`${pragma}
.renderToStaticMarkup`] = ['0.14.0', 'ReactDOMServer.renderToStaticMarkup']
  // 15.0.0
  deprecated[`${pragma}
.addons.LinkedStateMixin`] = ['15.0.0']
  ...
  // 15.5.0
  deprecated[`${pragma}
.createClass`] = ['15.5.0', 'the npm module create-react-class']
  deprecated[`${pragma}
.addons.TestUtils`] = ['15.5.0', 'ReactDOM.TestUtils']
  deprecated[`${pragma}
.PropTypes`] = ['15.5.0', 'the npm module prop-types']
  // 15.6.0
  deprecated[`${pragma}
.DOM`] = ['15.6.0', 'the npm module react-dom-factories']
  // 16.9.0
...
} is deprecated since React {{version}}{{newMethod}}{{refs}}',
}
...
const version = deprecated[methodName][0]
      ...
      report(context, messages.deprecated, 'deprecated', {
        ...
        data: {
          ...
          version,
          newMethod: newMethod ? `, use ${newMethod}
instead` : '',
          refs: refs ? `, see ${refs}
` : '',
        }
...
node.specifiers.filter((s => 'imported' in s && s.imported)).forEach(specifier => {
          ...
          checkDeprecation(node, 'imported' in specifier && 'name' in specifier.imported && `${MODULES[node.source.value][0]}
...
('properties' in node.id ? node.id.properties : undefined).filter(p => p.type !== 'RestElement' && p.key).forEach(property => {
          checkDeprecation(
            node,
            'key' in property && 'name' in property.key && `${reactModuleName || pragma}
.${property.key.name}`,
            property
          )
        }
