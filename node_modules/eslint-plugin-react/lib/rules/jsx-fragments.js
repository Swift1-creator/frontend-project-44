...
function replaceNode(source, node, text) {
  return `${source.slice(0, node.range[0])}
${text}${source.slice(node.range[1])}`
}
const messages = {
  ...
  preferPragma: 'Prefer {{react}
}.{{fragment}} over fragment shorthand',
  preferFragment: 'Prefer fragment shorthand over {{react}
}.{{fragment}}',
}
...
,
  ...
  create(context) {
    ...
    const openFragLong = `<${reactPragma}
.${fragmentPragma}>`
    const closeFragLong = `</${reactPragma}
.${fragmentPragma}>`
...
const jsxElements = []
    const fragmentNames = new Set([`${reactPragma}
.${fragmentPragma}`])
