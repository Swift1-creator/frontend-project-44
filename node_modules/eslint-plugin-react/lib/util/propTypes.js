 ...
 * @returns {boolean}
True if the node is a class with generic prop types, false if not.
 */
 ...
 * @param {Object}
context Array of properties to iterate.
 * @param {Object[]}
properties Array of properties to iterate.
 * @param {Function}
fn Function to call on each property, receives property key
    and property value. (key, value) => void
  * @param {Function}
[handleSpreadFn] Function to call on each ObjectTypeSpreadProperty, receives the
    argument
 */
...
node the AST node being checked.
 * @returns {boolean}
True if the node has a ClassBody ancestor, false if not.
 */
...
const genericReactTypesImport = new Set()
  // import { FC as X }
from 'react' -> localToImportedMap = { x: FC }
...
/**
   ...
   * @param {string}
key The name of the identifier to access.
   * @returns {ASTNode}
The ASTNode associated with the given identifier.
   */
...
key The name of the identifier to access
   * @param {ASTNode}
value The new value for the identifier.
   * @returns {ASTNode}
The ASTNode associated with the given identifier.
   */
...
/**
   ...
   * @param {string}
validator Name of validator to check.
   * @returns {boolean}
True if validator should be checked by custom validator.
   */
...
/* eslint-enable no-use-before-define */
   ...
   *
   * @param {ASTNode}
node The annotation or a node containing the type annotation.
   * @returns {ASTNode}
The resolved type annotation for the node.
   */
...
/**
   ...
   * @param {ASTNode}
annotation Type annotation for the props class property.
   * @param {string}
parentName
   * @param {Set<ASTNode>}
[seen]
   * @return {Object}
The representation of the declaration, empty object means
...
declaredPropTypes
   * @returns {boolean}
True if propTypes should be ignored (e.g. when a type can't be resolved, when it is imported)
   */
...
rootNode
   * @param {Function}
callback called with the resolved value only if resolved.
   */
...
/**
   ...
   * The representation is used to verify nested used properties.
   * @param {ASTNode}
value Node of the PropTypes for the desired property
   * @param {string}
parentName
   * @param {ASTNode}
rootNode
...
node
    ...
    traverseDeclaredInterfaceOrTypeAlias(node) {
      if (astUtil.isTSInterfaceDeclaration(node)) {
        // Handle TSInterfaceDeclaration interface Props { name: string, id: number}
, should put in properties list directly
...
node The AST node being checked.
   * @param {ASTNode}
propTypes The AST node containing the proptypes
   * @param {ASTNode}
rootNode
   */
...
/**
   * @param {ASTNode}
node We expect either an ArrowFunctionExpression,
...
node The AST node being checked.
   * @returns {boolean}
True if the node is a type annotated props declaration, false if not.
   */
