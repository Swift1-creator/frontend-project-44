...
function convertConfVerToSemver(confVer) {
  const fullSemverString = /^[0-9]+\.[0-9]+$/.test(confVer) ? `${confVer}
.0` : confVer
...
settingsDefaultVersion = String(settingsDefaultVersion)
    ...
    if (result) {
      ...
      error(`Warning: React version specified in eslint-plugin-react-settings must be a valid semver version, or "detect"; got “${settingsDefaultVersion}
”. Falling back to latest version as default.`)
    }
} else {
    defaultVersion = ULTIMATE_LATEST_SEMVER
  }
