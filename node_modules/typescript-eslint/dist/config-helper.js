...
const name = (() => {
            if ('name' in configWithExtends && configWithExtends.name != null) {
                if (typeof configWithExtends.name !== 'string') {
                    throw new Error(`tseslint.config(): Config at index ${configIndex}
has a 'name' property that is not a string.`)
                }
...
return undefined
        ...
        const nameErrorPhrase = name != null ? `, named "${name}
",` : ' (anonymous)'
        if (!Array.isArray(extendsArr)) {
            throw new TypeError(`tseslint.config(): Config at index ${configIndex}
${nameErrorPhrase} has an 'extends' property that is not an array.`)
        }
const extendsArrFlattened = extendsArr.flat(Infinity)
        ...
        for (const [extensionIndex, extension] of extendsArrFlattened.entries()) {
            ...
            if (typeof extension === 'string') {
                throw new Error(`tseslint.config(): Config at index ${configIndex}
${nameErrorPhrase} has an 'extends' array that contains a string (${JSON.stringify(extension)}) at index ${extensionIndex}.` +
...
// https://github.com/eslint/rewrite/blob/82d07fd0e8e06780b552a41f8bcbe2a4f8741d42/packages/config-helpers/src/define-config.js#L448-L450
            if ('basePath' in extension) {
                throw new TypeError(`tseslint.config(): Config at index ${configIndex}
${nameErrorPhrase} has an 'extends' array that contains a config with a 'basePath' property at index ${extensionIndex}.` +
                    ` 'basePath' in 'extends' is not allowed.`)
            }
if ('extends' in extension) {
                throw new TypeError(`tseslint.config(): Config at index ${configIndex}
${nameErrorPhrase} has an 'extends' array that contains a config with an 'extends' property at index ${extensionIndex}.` +
        ...
        if (nonObjectExtensions.length > 0) {
            ...
            throw new TypeError(`tseslint.config(): Config at index ${configIndex}
${nameErrorPhrase} contains non-object` +
