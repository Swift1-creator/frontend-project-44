...
/** @typedef {$eslintcore.ConfigObject}
Config */
/** @typedef {$eslintcore.LegacyConfigObject}
LegacyConfig */
/** @typedef {$eslintcore.Plugin}
Plugin */
/** @typedef {$eslintcore.RuleConfig}
RuleConfig */
/** @import * as $typests from "./types.ts"; */
/** @typedef {$typests.ExtendsElement}
ExtendsElement */
/** @typedef {$typests.SimpleExtendsElement}
SimpleExtendsElement */
/** @typedef {$typests.ConfigWithExtends}
ConfigWithExtends */
/** @typedef {$typests.InfiniteArray<Config>}
InfiniteConfigArray */
/** @typedef {$typests.ConfigWithExtendsArray}
 ...
 * @param {string}
indexPath The index path of the config object.
 * @return {string}
 ...
 * @param {SimpleExtendsElement}
extension The extension.
 * @param {string}
indexPath The index of the extension.
 * @return {string}
 ...
 * @return {config is LegacyConfig}
`true` if the config object is a legacy config.
 */
...
config The config object to check.
 * @return {boolean}
`true` if the config object is a global ignores config.
 */
 ...
 * @param {string}
userNamespace The namespace of the plugin.
 * @param {Plugin}
plugin The plugin config object.
 * @param {Config}
config The config object to normalize.
 * @return {Config}
...
=
			...
			if (ruleNamespace === pluginNamespace) {
				newRules[`${userNamespace}
/${ruleName}`] = result.rules[ruleId]
			}
else {
...
=
		...
		if (processorNamespace) {
			if (processorNamespace === pluginNamespace) {
				result.processor = `${userNamespace}
/${processorName}`
			}
}
	}
 ...
 * @param {string}
userPluginNamespace The namespace of the plugin.
 * @param {Plugin}
plugin The plugin object.
 * @param {Config|LegacyConfig|(Config|LegacyConfig)[]}
pluginConfig The plugin config to normalize.
 * @param {string}
pluginConfigName The name of the plugin config.
 * @return {InfiniteConfigArray}
The normalized plugin config.
 * @throws {TypeError}
If the plugin config is a legacy config.
 */
...
// if it's a legacy config, throw an error
	if (isLegacyConfig(pluginConfig)) {
		throw new TypeError(
			`Plugin config "${pluginConfigName}
" is an eslintrc config and cannot be used in this context.`,
		)
	}
...
The plugin config.
 * @throws {TypeError}
If the plugin config is not found or is a legacy config.
 */
...
=
	...
	if (!plugin) {
		throw new TypeError(`Plugin "${userPluginNamespace}
" not found.`)
	}
...
// If we get here, then the config was either not found or is a legacy config
	const message =
		directConfig || flatConfig
			? `Plugin config "${configName}
" in plugin "${userPluginNamespace}" is an eslintrc config and cannot be used in this context.`
			: `Plugin config "${configName}
" not found in plugin "${userPluginNamespace}".`
	throw new TypeError(message)
}
/**
 * Flattens an array while keeping track of the index path.
 * @param {any[]}
configList The array to traverse.
 * @param {string}
indexPath The index path of the value in a multidimensional array.
 * @return {IterableIterator<{indexPath:string, value:any}
>} The flattened list of values.
 */
function* flatTraverse(configList, indexPath = "") {
	for (let i = 0; i < configList.length; i++) {
		const newIndexPath = indexPath ? `${indexPath}
[${i}]` : `[${i}]`
 ...
 * @param {(string|string[])[]}
[baseFiles] The base files.
 * @param {(string|string[])[]}
[extensionFiles] The extension files.
 ...
 * @param {Config}
baseConfig The base config object.
 * @param {string}
baseConfigName The name of the base config object.
 * @param {Config}
extension The extension config object.
 * @param {string}
extensionName The index of the extension config object.
 * @return {Config}
 ...
 * @param {ConfigWithExtends}
config The config object.
 * @param {WeakMap<Config, string>}
configNames The map of config objects to their names.
 * @return {Config[]}
The flattened list of config objects.
 * @throws {TypeError}
If the `extends` property is not an array or if nested `extends` is found.
 */
...
return /** @type {Config} */ (extendsElement)
	...
	for (const { indexPath, value: extendsElement }
of flatTraverse(
		objectExtends,
	)) {
		const extension = /** @type {Config}
*/ (extendsElement)
 ...
 * @param {ConfigWithExtends[]}
configList The list of config objects and arrays.
 * @param {WeakMap<Config, string>}
configNames The map of config objects to their names.
 * @return {Config[]}
The flattened list of config objects.
...
// first flatten the list of configs and get the names
	for (const { indexPath, value }
of flatTraverse(args)) {
 ...
 * @param {string[]}
ignorePatterns The ignore patterns.
 * @param {string}
[name] The name of the global ignores config.
 * @returns {Config}
The global ignores config.
 * @throws {TypeError}
If ignorePatterns is not an array or if it is empty.
 */
