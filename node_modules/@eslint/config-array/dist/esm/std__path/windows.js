 ...
 * ```ts
 * import { fromFileUrl }
from "@std/path/windows/from-file-url"
 * import { assertEquals }
from "@std/assert"
 *
 ...
 */ function fromFileUrl(url) {
  url = assertArg$3(url)
  let path = decodeURIComponent(url.pathname.replace(/\//g, "\\").replace(/%(?![0-9A-Fa-f]{2}
)/g, "%25")).replace(/^\\*([A-Za-z]:)(\\|$)/, "$1\\")
  if (url.hostname !== "") {
 ...
 * ```ts
 * import { basename }
from "@std/path/windows/basename"
 * import { assertEquals }
from "@std/assert"
 *
 ...
 * ```ts
 * import { isAbsolute }
from "@std/path/windows/is-absolute"
 * import { assert, assertFalse }
from "@std/assert"
 *
 ...
 * ```ts
 * import { normalize }
from "@std/path/windows/normalize"
 * import { assertEquals }
from "@std/assert"
 *
...
return tail
    ...
    if (tail.length > 0) return `${device}
\\${tail}`
    else return `${device}
\\`
  }
...
if (!resolvedAbsolute) {
      resolvedTail = `${path.slice(rootEnd)}
\\${resolvedTail}`
 ...
 * ```ts
 * import { relative }
from "@std/path/windows/relative"
 * import { assertEquals }
from "@std/assert"
 *
 ...
 * ```ts
 * import { toFileUrl }
from "@std/path/windows/to-file-url"
 * import { assertEquals }
from "@std/assert"
 *
 ...
 * @example Usage
 * ```ts
 * import { toNamespacedPath }
from "@std/path/windows/to-namespaced-path"
 * import { assertEquals }
from "@std/assert"
 *
...
, {@linkcode joinGlobs},
 * {@linkcode normalizeGlob}
and {@linkcode expandGlob}.
  ...
  for(let j = 0; j < glob.length;){
    ...
    for(; i < glob.length && !c.seps.includes(glob[i]); i++){
      if (inEscape) {
        ...
        segment += escapeChars.includes(glob[i]) ? `\\${glob[i]}
` : glob[i]
        continue
      }
...
// Check for unclosed groups or a dangling backslash.
    if (groupStack.length > 0 || inRange || inEscape) {
      ...
      for (const c of glob.slice(j, i)){
        segment += REG_EXP_ESCAPE_CHARS.includes(c) ? `\\${c}
` : c
        endsWithSep = false
      }
}
...
`.
 * - `@(foo|bar)` - Matches 1 instance of `{foo,bar}
 ...
 * ```ts
 * import { globToRegExp }
from "@std/path/windows/glob-to-regexp"
 * import { assertEquals }
from "@std/assert"
 *
 ...
 * @example Usage
 * ```ts
 * import { normalizeGlob }
from "@std/path/windows/normalize-glob"
 * import { assertEquals }
from "@std/assert"
 *
...
const s = SEPARATOR_PATTERN.source
  const badParentPattern = new RegExp(`(?<=(${s}
|^)\\*\\*${s})\\.\\.(?=${s}|$)`, "g")
 ...
 * ```ts
 * import { joinGlobs }
from "@std/path/windows/join-globs"
 * import { assertEquals }
from "@std/assert"
 *
