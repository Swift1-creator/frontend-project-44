  ...
  if (propertyName.kind === ts9.SyntaxKind.ComputedPropertyName) {
    ...
    if (ts9.isPrefixUnaryExpression(expression)) {
      ...
      switch (expression.operator) {
        ...
        // falls through
        case ts9.SyntaxKind.PlusToken:
          return ts9.isNumericLiteral(expression.operand) ? `${negate ? "-" : ""}
${expression.operand.text}` : ts9.isBigIntLiteral(expression.operand) ? `${negate ? "-" : ""}${expression.operand.text.slice(0, -1)}` : undefined
        default:
          return
      }
}
