...
function openClientFileFromProjectService(defaultProjectMatchedFiles, isDefaultProjectAllowed, filePathAbsolute, parseSettings, serviceAndSettings) {
    ...
    if (opened.configFileName) {
        if (isDefaultProjectAllowed) {
            throw new Error(`${parseSettings.filePath}
was included by allowDefaultProject but also was found in the project service. Consider removing it from allowDefaultProject.`)
        }
}
    else {
        const wasNotFound = `${parseSettings.filePath}
was not found by the project service`
        ...
        if (!shared_1.DEFAULT_EXTRA_FILE_EXTENSIONS.has(fileExtension) &&
            ...
            const nonStandardExt = `${wasNotFound}
because the extension for the file (\`${fileExtension}\`) is non-standard`
            if (extraFileExtensions.length > 0) {
                throw new Error(`${nonStandardExt}
...
const relativeFilePath = node_path_1.default.relative(parseSettings.tsconfigRootDir, filePathAbsolute)
            throw new Error([
                baseMessage,
                `allowDefaultProject is set to ${JSON.stringify(allowDefaultProject)}
, which does not match '${relativeFilePath}'.`,
            ].join('\n'))
        }
}
    ...
    if (!opened.configFileName) {
        ...
        if (defaultProjectMatchedFiles.size >
            ...
            throw new Error(`Too many files (>${serviceAndSettings.maximumDefaultProjectFileMatchCount}
) have matched the default project.${validateDefaultProjectForFilesGlob_1.DEFAULT_PROJECT_FILES_ERROR_EXPLANATION}
Matching files:
${filesToPrint.map(file => `- ${file}
`).join('\n')}
${truncatedFileCount ? `...and ${truncatedFileCount}
more files\n` : ''}
