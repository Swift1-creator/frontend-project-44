    ...
    #checkForStatementDeclaration(initializer, kind) {
        ...
        if (ts.isVariableDeclarationList(initializer)) {
            if (initializer.declarations.length !== 1) {
                this.#throwError(initializer, `Only a single variable declaration is allowed in a '${loop}
' statement.`)
            }
const declaration = initializer.declarations[0]
            if (declaration.initializer) {
                this.#throwError(declaration, `The variable declaration of a '${loop}
...
if (kind === ts.SyntaxKind.ForInStatement &&
            ...
            this.#throwError(initializer, `The left-hand side of a '${loop}
    ...
    #withDeprecatedAliasGetter(node, aliasKey, valueKey, suppressWarnings = false) {
        ...
        Object.defineProperty(node, aliasKey, {
            ...
            get: this.options.suppressDeprecatedPropertyWarnings
                ...
                : () => {
                    if (!warned) {
                        process.emitWarning(`The '${aliasKey}
' property is deprecated on ${node.type} nodes. Use '${valueKey}' instead. See https://typescript-eslint.io/troubleshooting/faqs/general#the-key-property-is-deprecated-on-type-nodes-use-key-instead-warnings.`, 'DeprecationWarning')
                        warned = true
                    }
...
#withDeprecatedGetter(node, deprecatedKey, preferredKey, value) {
        ...
        Object.defineProperty(node, deprecatedKey, {
            ...
            get: this.options.suppressDeprecatedPropertyWarnings
                ...
                : () => {
                    if (!warned) {
                        let message = `The '${deprecatedKey}
' property is deprecated on ${node.type} nodes.`
                        if (preferredKey) {
                            message += ` Use ${preferredKey}
instead.`
                        }
message +=
...
if (result.kind === 'using' || result.kind === 'await using') {
                    node.declarationList.declarations.forEach((declaration, i) => {
                        if (result.declarations[i].init == null) {
                            this.#throwError(declaration, `'${result.kind}
' declarations must be initialized.`)
                        }
if (result.declarations[i].id.type !== ts_estree_1.AST_NODE_TYPES.Identifier) {
                            this.#throwError(declaration.name, `'${result.kind}
' declarations may not have binding patterns.`)
                        }
})
                }
            ...
            case SyntaxKind.VariableDeclarationList: {
                ...
                if (result.kind === 'using' || result.kind === 'await using') {
                    node.declarations.forEach((declaration, i) => {
                        if (result.declarations[i].init != null) {
                            this.#throwError(declaration, `'${result.kind}
' declarations may not be initialized in for statement.`)
                        }
if (result.declarations[i].id.type !== ts_estree_1.AST_NODE_TYPES.Identifier) {
                            this.#throwError(declaration.name, `'${result.kind}
' declarations may not have binding patterns.`)
...
case SyntaxKind.PropertyAssignment: {
                // eslint-disable-next-line @typescript-eslint/no-deprecated
                const { exclamationToken, questionToken }
= node
                if (questionToken) {
...
case SyntaxKind.ShorthandPropertyAssignment: {
                // eslint-disable-next-line @typescript-eslint/no-deprecated
                const { exclamationToken, modifiers, questionToken }
= node
                if (modifiers) {
            ...
            case SyntaxKind.MethodDeclaration: {
                ...
                if (isAbstract && node.body) {
                    this.#throwError(node.name, node.kind === SyntaxKind.GetAccessor ||
                        ...
                        : `Method '${(0, node_utils_1.declarationNameToString)(node.name, this.ast)}
...
= heritageClause
                    if (types.length === 0) {
                        this.#throwUnlessAllowInvalidAST(heritageClause, `'${ts.tokenToString(token)}
