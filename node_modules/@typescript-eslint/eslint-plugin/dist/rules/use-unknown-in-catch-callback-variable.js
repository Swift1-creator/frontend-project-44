...
const useUnknownMessageBase = 'Prefer the safe `: unknown` for a `{{method}
}`{{append}} callback variable.'
...
,
        ...
        messages: {
            ...
            useUnknownArrayDestructuringPattern: `${useUnknownMessageBase}
The thrown error may not be iterable.`,
...
) => staticMemberAccessKey === method)
                if (!promiseMethodInfo) {
                    return
                }
// Need to be enough args to check
                const { argIndexToCheck, ...data }
= promiseMethodInfo
...
// Argument to check, and all arguments before it, must be "ordinary" arguments (i.e. no spread arguments)
                // promise.catch(f), promise.catch(() => {}
), promise.catch(<expression>, <<other-args>>)
                const argsToCheck = args.slice(0, argIndexToCheck + 1)
                if (argsToCheck.some(({ type }
) => type === utils_1.AST_NODE_TYPES.SpreadElement)) {
                    return
                }
