...
function endsWithUnescapedDollarSign(str) {
    return new RegExp(`${evenNumOfBackslashesRegExp.source}
\\$$`).test(str)
}
...
function hasCommentsBetweenQuasi(startQuasi, endQuasi) {
            ...
            const endToken = (0, util_1.nullThrows)(context.sourceCode.getTokenByRangeStart(endQuasi.range[0]), util_1.NullThrowsReasons.MissingToken('}
...
function getReportDescriptors(infos) {
            ...
            for (const { interpolation, nextQuasi, prevQuasi }
of reversedInfos) {
                const fixers = []
...
const literal = getLiteral(interpolation)
                ...
                if (literal) {
                    let escapedValue = (typeof literal.value === 'string'
                        ...
                        // \${ -> \${
                        // \\${ -> \\\${
                        .replaceAll(new RegExp(`${evenNumOfBackslashesRegExp.source}
(\`|\\\${)`, 'g'), '\\$1')
                    // `...${'...$'}
{...`
                    //           ^^^^
...
= (0, util_1.getConstraintInfo)(checker, services.getTypeAtLocation(node.expressions[0]))
...
= (0, util_1.getConstraintInfo)(checker, services.getTypeAtLocation(node.types[0]))
                    if (constraintType &&
                        !isTypeParameter &&
