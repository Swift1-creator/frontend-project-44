...
,
        ...
        messages: {
            ...
            someImportsAreOnlyTypes: 'Imports {{typeImports}
} are only used as type.',
...
child = parent
                                        ...
                                        // CASE 2:
                                        // `type T = { [foo]: string }
` will create a value reference because "foo" must be a value type
                                        // however this value reference is safe to use with type-only imports.
...
function classifySpecifier(node) {
        ...
        function getFixesNamedSpecifiers(fixer, node, subsetNamedSpecifiers, allNamedSpecifiers) {
            ...
            if (subsetNamedSpecifiers.length === allNamedSpecifiers.length) {
                // import Foo, {Type1, Type2}
from 'foo'
                // import DefType, {Type1, Type2}
from 'foo'
                ...
                const closingBraceToken = (0, util_1.nullThrows)(context.sourceCode.getFirstTokenBetween(openingBraceToken, node.source, util_1.isClosingBraceToken), util_1.NullThrowsReasons.MissingToken('}
', node.type))
                // import DefType, {...}
from 'foo'
                //               ^^^^^^^ remove
...
return {
         ...
         * e.g.
         * import type { Already, Type1, Type2 }
from 'foo'
         *                        ^^^^^^^^^^^^^ insert
         */
        function fixInsertNamedSpecifiersInNamedSpecifierList(fixer, target, insertText) {
            const closingBraceToken = (0, util_1.nullThrows)(context.sourceCode.getFirstTokenBetween((0, util_1.nullThrows)(context.sourceCode.getFirstToken(target), util_1.NullThrowsReasons.MissingToken('token before', 'import')), target.source, util_1.isClosingBraceToken), util_1.NullThrowsReasons.MissingToken('}
', target.type))
...
/**
         ...
         * e.g.
         * import ADefault, { Already, type Type1, type Type2 }
from 'foo'
         *                             ^^^^ insert
         */
...
if (fixStyle === 'inline-type-imports' &&
                    ...
                    // import AValue, {BValue, Type1, Type2}
from 'foo'
...
`
                        ...
                        yield fixer.insertTextBefore(node, `import type {${fixesNamedSpecifiers.typeNamedSpecifiersText}
} from ${context.sourceCode.getText(node.source)};\n`)
                    }
}
            }
const fixesRemoveTypeNamespaceSpecifier = []
            if (namespaceSpecifier &&
                ...
                // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ insert
                yield fixer.insertTextBefore(node, `import type ${context.sourceCode.getText(namespaceSpecifier)}
from ${context.sourceCode.getText(node.source)};\n`)
            }
if (defaultSpecifier &&
                report.typeSpecifiers.includes(defaultSpecifier)) {
...
from 'foo'
                    ...
                    const defaultText = context.sourceCode.text
                        ...
                        .trim()
                    yield fixer.insertTextBefore(node, `import type ${defaultText}
from ${context.sourceCode.getText(node.source)};\n`)
...
function* fixRemoveTypeSpecifierFromImportDeclaration(fixer, node) {
            ...
            const afterToken = (0, util_1.nullThrows)(context.sourceCode.getTokenAfter(typeToken, { includeComments: true }
), util_1.NullThrowsReasons.MissingToken('any token', node.type))
