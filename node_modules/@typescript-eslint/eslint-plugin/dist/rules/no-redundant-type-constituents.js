...
,
        messages: {
            errorTypeOverrides: `'{{typeName}
}' is an 'error' type that acts as 'any' and overrides all other types in this {{container}} type.`,
            literalOverridden: `{{literal}
} is overridden by {{primitive}} in this union type.`,
            overridden: `'{{typeName}
}' is overridden by other types in this {{container}} type.`,
            overrides: `'{{typeName}
}' overrides all other types in this {{container}} type.`,
            primitiveOverridden: `{{primitive}
} is overridden by the {{literal}} in this intersection type.`,
        }
,
        schema: [],
    }
...
return {
            'TSIntersectionType:exit'(node) {
                ...
                function checkIntersectionBottomAndTopTypes({ typeFlags, typeName }
, typeNode) {
...
,
            ...
            'TSUnionType:exit'(node) {
                ...
                function checkUnionBottomAndTopTypes({ typeFlags, typeName }
, typeNode) {
...
for (const literalTypeFlag of literalTypeFlags) {
                ...
                for (const [primitiveTypeFlag, typeNodesWithText] of seenLiteralTypes) {
                    if (seenPrimitiveTypes.has(primitiveTypeFlag)) {
                        for (const { literalValue, typeNode }
